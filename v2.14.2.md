...
Rancher v2.14.2 is the latest patch release of Rancher. This is a Community version release that introduces maintenance updates and bug fixes.

For more information on new features in the general minor release, see the [v2.14.0 release notes](https://github.com/rancher/rancher/releases/tag/v2.14.0).
...
- Fixed a security vulnerability in the GitHub App authentication provider where users incorrectly inherited permissions from all teams within their GitHub organization, rather than only the specific teams to which they belonged. Upon upgrading, Rancher automatically triggers a mandatory refresh of all affected user `principals` to remove these incorrectly assigned team memberships and restore proper access. For more information, refer to [CVE-2026-41053](https://github.com/rancher/rancher/security/advisories/GHSA-4j6x-2764-m8gh).

- Updated the permissions of the built-in `project-owner` role to no longer include the `updatepsa` verb. This prevents users with this role from bypassing restricted PSA policies or deploying privileged workloads within their projects. If your organization requires users to retain this capability, administrators must create a custom project role that explicitly grants the `updatepsa` verb for the project resource. For more information, refer to [CVE-2026-41052](https://github.com/rancher/rancher/security/advisories/GHSA-vx8h-4prv-g744).

- Rancher now validates the `authImage` parameter in cluster import manifests to prevent YAML injection attacks. For more information, refer to [CVE-2026-44939](https://github.com/rancher/rancher/security/advisories/GHSA-mhc6-2gfq-xx62).

### Fleet

- Resolved a conceptual security issue for multi-tenant and multi-user single-cluster environments by introducing a `Policy` resource. If you have been using `GitRepoRestriction`, please migrate to `Policy`. For more information, refer to [CVE-2026-44935](https://github.com/rancher/fleet/security/advisories/GHSA-xr65-5cpm-g36x).

- Fleet now mandates that `helmRepoURLRegex` be set on the `GitRepo` resource before Helm repository credentials are forwarded to chart repository URLs specified in `fleet.yaml`. Existing `GitRepo` resources that have a Helm secret configured are automatically migrated upon upgrade to preserve their previous behavior. For more information, refer to [CVE-2026-44936](https://github.com/rancher/fleet/security/advisories/GHSA-hx4v-cxpf-vh8m).

- Fixed a security vulnerability in the webhook component that enabled attackers to forge webhooks, which Fleet would then process if webhook shared secrets were not configured. For more information, refer to [CVE-2026-44937](https://github.com/rancher/fleet/security/advisories/GHSA-jmf4-m7j9-g72r).

- Fleet now prevents `namespaceLabels` in `fleet.yaml` from overwriting Pod Security Standards enforcement labels on existing namespaces. For more information, refer to [CVE-2026-44938](https://github.com/rancher/fleet/security/advisories/GHSA-864g-863m-vcvq).

### SUSE Observability

- Fixed a security vulnerability that exposed service tokens by migrating the token to a secure Kubernetes Secret that is automatically deleted when the extension is uninstalled. For more information refer to [CVE-2026-44940](https://github.com/StackVista/rancher-extension-stackstate/security/advisories/GHSA-7c27-jc6w-pw95).

### SUSE Rancher AI Assistant

- Fixed an information disclosure vulnerability where sensitive configuration secrets and MCP responses were exposed when the logging level was set to \`DEBUG. For more information, refer to [CVE-2026-44934](https://github.com/rancher/rancher-ai-agent/security/advisories/GHSA-5r2r-h824-fr5v).

For additional details, see the Security Advisories and CVEs page in Rancher’s [documentation](security/cves.xml) or in Rancher’s GitHub [repository](https://github.com/rancher/rancher/security/advisories).

## Rancher General

### Known Issues

- A known issue can cause Rancher Logging DaemonSets (`rancher-logging-rke2-journald-aggregator` and `rancher-logging-root-fluentbit`) to enter `CrashLoopBackOff` or remain unstable when host `inotify` limits are too low. As a workaround, increase `fs.inotify.max_user_watches` and `fs.inotify.max_user_instances` on affected nodes. See [\#54216](https://github.com/rancher/rancher/issues/54216).

## Rancher App (Global UI)

### Major Bug Fixes

- It is now possible to install the same chart multiple times in the same cluster from **Apps** \> **Charts** by using **Install as a new instance**. See [\#16927](https://github.com/rancher/dashboard/issues/16927) and [\#16907](https://github.com/rancher/dashboard/issues/16907).

- Fixed an issue where add-on chart values could be lost when viewing or editing cluster configuration in the Rancher UI. See [\#17527](https://github.com/rancher/dashboard/issues/17527) and [\#17498](https://github.com/rancher/dashboard/issues/17498).

## Changes Since v2.14.1

See the full list of [changes](https://github.com/rancher/rancher/compare/v2.14.1%E2%80%A6v2.14.2).
...
- CLI - [v2.14.2](https://github.com/rancher/cli/releases/tag/v2.14.2)
...
- **Rancher v2.14.1:**
  ...
  - **Deprecation:** Starting from Rancher v2.14.1, Cluster API Addon Provider Fleet (CAAPF) is being disabled by default and will be removed in a future Rancher release per our [deprecation policy](https://www.suse.com/support/rancher-prime/#Rancher-Prime-Deprecation-Policy). An automated migration script for CAAPF resources is available when upgrading to Rancher v2.14.1, refer to the [CAPI documentation](https://turtles.docs.rancher.com/turtles/stable/en/user/caapf.html#_migrating_existing_caapf_managed_clusters_to_rancher). While standard Fleet integration is still available through Rancher, additional steps are needed by users that upgrade to Rancher v2.14.1 and are running CAPI clusters that are provisioned using CAAPF. If you are upgrading CAPI clusters provisioned with CAAPF, keep `features.use-caapf.enabled=true` before upgrading to avoid Fleet-management handoff conflicts that can disrupt downstream workloads. Refer to [Features](https://turtles.docs.rancher.com/turtles/stable/en/overview/features.html#_how_to_set_feature_gates) and the [Cluster API Addon Provider Fleet](https://turtles.docs.rancher.com/turtles/stable/en/user/caapf.html) documentation for more information. See [\#54740](https://github.com/rancher/rancher/issues/54740).
