...
ifeval::["{build-type}" == "srfa"]
Hosted cluster:: A managed cluster that is hosted in the cloud, by a Kubernetes provider. Rancher supports Amazon Elastic Kubernetes Service (EKS) and integrates with the hosted Kubernetes provider's cloud APIs, so that you can manage the cluster from the Rancher UI.
endif::[]
+
