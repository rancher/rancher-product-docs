...
. If you choose to log out of your IdP, provide an <<How to get the End Session Endpoint,*End Session Endpoint*>>. Rancher uses this URL to initiate the external logout.
...
. Obtain the Discovery URL by appending the IdP Issuer URL with the well-known path (`.well-known/openid-configuration`).
. Send an HTTP `GET` request to the Discovery URL.
. In the JSON object, look for the key named `end_session_endpoint` and retrieve the URL.
+
