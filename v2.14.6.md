...
Rancher v2.14.6 is the latest patch release of Rancher. This is a Prime version release that introduces maintenance updates and bug fixes. To learn more about Rancher Prime, see our page on the [Rancher Prime Platform](https://www.rancher.com/products/rancher-platform).
...
- Fixed an issue where the Fleet agent copied the Secrets and ConfigMaps listed in a Bundle’s `downstreamResources`, and created the deployment’s target namespace, using the agent’s own credentials instead of the ServiceAccount pinned to the deployment. Those writes are now gated by the tenant’s downstream RBAC. Deployments that do not pin a ServiceAccount are unchanged as an agent operation. On Rancher v2.13.x and v2.14.x the copy is behind `EXPERIMENTAL_COPY_RESOURCES_DOWNSTREAM` and disabled by default, so only installations that explicitly enabled it were exposed. After upgrading, a deployment that pins a ServiceAccount without permission to create the target namespace, or to write the copied Secrets and ConfigMaps in it, will no longer deploy. It reports “not ready” and retries until the permission is granted. Grant the ServiceAccount the missing permissions, or pre-create the target namespace. For more information, refer to [CVE-2026-88808](https://github.com/rancher/fleet/security/advisories/GHSA-q9v4-358v-r8q5).

- Fixed a security vulnerability in Fleet where namespace labels and annotations requested by a bundle through `namespaceLabels` and `namespaceAnnotations` were applied without the authorization of the service account the bundle was pinned to. In multi-tenant deployments this allowed a bundle to modify namespace metadata that its own identity was not permitted to change. The namespace update now runs as the deployment’s service account and is governed by downstream Kubernetes RBAC. For more information, refer to [CVE-2026-93540](https://github.com/rancher/fleet/security/advisories/GHSA-m93g-8438-2cgg).

## Authentication

### Major Bug Fixes

- The ext.Kubeconfig store has been updated to align with standard Kubernetes API resource behavior. See [\#56342](https://github.com/rancher/rancher/issues/56342).

## Role-Based Access Control (RBAC)

### Major Bug Fixes

- Fixed the reconciler loop for Global Roles that contain the `inheritedFleetWorkspacePermissions` field. The fix prevents Rancher logs from being spammed with create/delete operations. See [\#57057](https://github.com/rancher/rancher/issues/57057).
...
- Extended Rancher to better protect its managed resources for limit ranges and resource quotas from interference by regular users.
  ...
  - The webhook now rejects attempts from outside of Rancher to edit these managed resources, to promote an unmanaged resource to managed, or to demote a managed resource to unmanaged, while still allowing Rancher itself full access. See [\#56774](https://github.com/rancher/rancher/issues/56774).

## Cluster Provisioning

### Major Bug Fixes

- Fixed an issue where the Rancher UI offered GKE Kubernetes versions that were not available in any GKE release channel. Rancher v2.14.6 now filters the version list so users cannot select GKE versions that fail to provision because Google requires new GKE clusters to use a release channel. See [\#18929](https://github.com/rancher/dashboard/issues/18929), and [\#56879](https://github.com/rancher/rancher/issues/56879).

## Changes Since v2.14.5

See the full list of [changes](https://github.com/rancher/rancher/compare/v2.14.5%E2%80%A6v2.14.6).
...
- rancher/rancher:v2.14.6

### Tools

- CLI - [v2.14.6](https://github.com/rancher/cli/releases/tag/v2.14.6)
