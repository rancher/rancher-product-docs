...
:rn-capi: https://ranchermanager.docs.rancher.com/{rn-component-version}/integrations-in-rancher/cluster-api/overview
:rn-capi-infra-providers: https://ranchermanager.docs.rancher.com/{rn-component-version}/how-to-guides/advanced-user-guides/capi-infrastructure-providers
...
:rn-existing-gateway: https://ranchermanager.docs.rancher.com/{rn-component-version}/how-to-guides/advanced-user-guides/rancher-deployment-guides/configure-with-existing-gateway
:rn-rollback-alternative: https://ranchermanager.docs.rancher.com/{rn-component-version}/getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/rollbacks#alternative-steps-for-special-scenarios
...
:rn-imagescan: https://fleet.rancher.io/0.15/how-tos-for-users/imagescan
:rn-fleet-enable-imagescan: https://fleet.rancher.io/0.15/how-tos-for-operators/installation#enabling-imagescan
:rn-fleet-helm4-drift-correction: https://fleet.rancher.io/0.15/changelogs/v0.15.0#_drift_correction_may_not_work_as_expected_with_helm_v4
...
:rn-capi: xref:integrations/cluster-api/overview.adoc
...
:rn-existing-gateway: xref:installation-and-upgrade/infrastructure-setup/configure-with-existing-gateway.adoc
:rn-rollback-alternative: xref:installation-and-upgrade/rollbacks.adoc#_alternative_steps_for_special_scenarios
...
:rn-fleet-enable-imagescan: https://documentation.suse.com/cloudnative/continuous-delivery/v0.15/en/how-tos-for-operators/installation.html#_enabling_imagescan
:rn-fleet-helm4-drift-correction: https://documentation.suse.com/cloudnative/continuous-delivery/v0.15/en/changelogs/v0.15.0.html#_drift_correction_may_not_work_as_expected_with_helm_v4
...
* Rancher v2.14 adds support for switching from ingress to Gateway API when installing the Rancher Helm chart. Note that when using the Gateway API network exposure type, Rancher can create and manage its own Gateway resource. However, if you have an existing Gateway that you manage independently (for example, a shared Gateway used by multiple applications), you will need to create your own HTTPRoute resources to route traffic to Rancher. For more information, refer to {rn-existing-gateway}[Using an External Gateway with Rancher]. See https://github.com/rancher/rancher/issues/52796[#52796].
...
- Rancher v2.14 adds support for Cluster API **v1beta2** by upgrading to **cluster-api v1.12.2** (from **v1.10.6**). Rolling back to **Rancher 2.13.x**, which supports Cluster API **v1beta1**, introduces compatibility issues and requires alternative rollback steps. Refer to the {rn-rollback-alternative}[Rancher documentation] for the required procedure. Refer to {rn-capi-v1beta2}[Upgrading to CAPI v1beta2] for more details on the changes introduced in this new API version. See https://github.com/rancher/rancher/issues/52034[#52034] and https://github.com/rancher/rancher/issues/53334[#53334].
...
+
For more information on installing and using Rancher Turtles, refer to the {rn-capi}[Cluster API integration documentation]. See https://github.com/rancher/rancher/issues/53291[#53291].
...
* When performing a rollback from Rancher v2.14 to Rancher v2.13 using the Backup/Restore Operator, the restore process requires all Rancher-related resources to be cleaned up on the upstream (local) cluster. For more information on the required rollback steps, refer to the {rn-rollback-alternative}[Rancher documentation]. See also the above release note on Cluster API **v1beta2**, introduced by the **cluster-api v1.12.2** upgrade, and https://github.com/rancher/backup-restore-operator/issues/916[#916].
...
* Under certain circumstances, drift correction in Fleet v0.15.0 may not work as expected with Helm v4. See the {rn-fleet-helm4-drift-correction}[v0.15.0 changelog] and https://github.com/rancher/fleet/issues/4878[#4878].
