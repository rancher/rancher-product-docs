...
=======
:revdate: 2026-01-26
>>>>>>> b14c8c99d (Archive community-docs/v2.9 & versions/v2.9)
:page-revdate: {revdate}
:description: 了解容器网络接口 (CNI)、Rancher 提供的 CNI 网络插件、提供商的功能，以及如何选择网络提供商

== 什么是 CNI？

CNI（容器网络接口）是一个link:https://cncf.io/[云原生计算基金会项目]，它包含了一些规范和库，用于编写在 Linux 容器中配置网络接口的一系列插件。CNI 只关注容器的网络连接，并在容器被删除时移除所分配的资源。
...
image::unencapsulated-network.png[非封装网络]
...
Kubernetes Worker 需要打开 UDP 端口 `8472` (VXLAN) 和 TCP 端口 `9099`（健康检查）。如果使用 Wireguard，则需要打开 UDP 端口 `51820` 和 `51821`。有关详细信息，请参阅xref:cluster-deployment/node-requirements.adoc[下游集群的端口要求]。

image::canal-diagram.png[]

For more information, refer to the https://github.com/rancher/rke2-charts/tree/main-source/packages/rke2-canal[Rancher maintained Canal source] and the https://github.com/projectcalico/canal[Canal GitHub Page].
...
Kubernetes Worker 需要打开 UDP 端口 `8472` (VXLAN)。有关详细信息，请参阅xref:cluster-deployment/node-requirements.adoc#_网络要求[下游集群的端口要求]。
...
Kubernetes worker 需要打开 TCP 端口 `6783`（控制端口）、UDP 端口 `6783` 和 UDP 端口 `6784`（数据端口）。有关详细信息，请参阅xref:cluster-deployment/node-requirements.adoc#_网络要求[下游集群的端口要求]。

有关详细信息，请参阅以下页面：

* https://github.com/weaveworks/weave/blob/master/site/overview.md[Weave Net 官网]

=== {rke2-product-name} Kubernetes 集群

Rancher 开箱即用地为 RKE2 Kubernetes 集群提供了几个 CNI 网络插件，分别是 <<_canal,Canal>>（见上一节）、Calico 和 Cilium。
...
如果使用 BGP，Kubernetes Worker 需要打开 TCP 端口 `179`，如果使用 VXLAN 封装，则需要打开 UDP 端口 `4789`。另外，使用 Typha 时需要 TCP 端口 `5473`。有关详细信息，请参阅xref:cluster-deployment/node-requirements.adoc#_网络要求[下游集群的端口要求]。
...
* 网络模型：封装或未封装。如需更多信息，请参阅 <<_cni_使用了哪些网络模型,CNI 中使用的网络模型>>。
...
include::shared:ROOT:partial$zh/cni-popularity.adoc[]
...
如需了解如何为你的集群配置网络插件，请参阅xref:cluster-deployment/configuration/rke1.adoc[集群选项]。有关更高级的配置选项，请参阅有关使用xref:cluster-deployment/configuration/rke1.adoc#_rke_集群配置文件参考[配置文件]和link:https://rancher.com/docs/rke/latest/en/config-options/add-ons/network-plugins/[网络插件]选项来配置集群的说明。
