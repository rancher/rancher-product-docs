...
:release-version: v2.14.6
:rn-component-version: v2.14
:previous-release-version: v2.14.5
...
On Rancher v2.13.x and v2.14.x the copy is behind `EXPERIMENTAL_COPY_RESOURCES_DOWNSTREAM` and disabled by default, so only installations that explicitly enabled it were exposed.
...
* Fixed a security vulnerability in Fleet where namespace labels and annotations requested by a bundle through `namespaceLabels` and `namespaceAnnotations` were applied without the authorization of the service account the bundle was pinned to. In multi-tenant deployments this allowed a bundle to modify namespace metadata that its own identity was not permitted to change. The namespace update now runs as the deployment's service account and is governed by downstream Kubernetes RBAC. For more information, refer to https://github.com/rancher/fleet/security/advisories/GHSA-m93g-8438-2cgg[CVE-2026-93540].
...
* The ext.Kubeconfig store has been updated to align with standard Kubernetes API resource behavior. See https://github.com/rancher/rancher/issues/56342[#56342].
...
* Fixed the reconciler loop for Global Roles that contain the `inheritedFleetWorkspacePermissions` field. The fix prevents Rancher logs from being spammed with create/delete operations. See https://github.com/rancher/rancher/issues/57057[#57057].
...
** The webhook now rejects attempts from outside of Rancher to edit these managed resources, to promote an unmanaged resource to managed, or to demote a managed resource to unmanaged, while still allowing Rancher itself full access. See https://github.com/rancher/rancher/issues/56774[#56774].
...
* Fixed an issue where the Rancher UI offered GKE Kubernetes versions that were not available in any GKE release channel. Rancher v2.14.6 now filters the version list so users cannot select GKE versions that fail to provision because Google requires new GKE clusters to use a release channel. See https://github.com/rancher/dashboard/issues/18929[#18929], and https://github.com/rancher/rancher/issues/56879[#56879].
