...
| `auditLog.level`
| 0
| `int` - set the {xref-enable-api-audit-log}[API Audit Log] level. 0 is off. [0-3]
|
...
| `restrictedAdmin`
| `false`
| `bool` - When this option is set to `true`, the initial Rancher user has restricted access to the local Kubernetes cluster to prevent privilege escalation. For more information, see the section about the {xref-global-permissions}#_restricted_admin[restricted-admin role].
|
