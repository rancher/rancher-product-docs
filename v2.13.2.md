...
- If you are using Active Directory Federation Service (AD FS),
  ...
  verification certificate from the metadata. For more information, see
  [\#48655](https://github.com/rancher/rancher/issues/48655). These
  issues can be corrected by either of two methods:
...
the [v2.13.0 release
notes](https://github.com/rancher/rancher/releases/tag/v2.13.0).
...
Rancher’s
[documentation](https://ranchermanager.docs.rancher.com/v2.13/reference-guides/rancher-security/security-advisories-and-cves)
or in Rancher’s GitHub
...
- Fixed an issue where the selection of pods in the Rancher UI
  ...
  See [\#16094](https://github.com/rancher/dashboard/issues/16094).
...
- Added additional improvements to reduce the amount of failed network
  ...
  [\#16216](https://github.com/rancher/dashboard/issues/16216).
...
- It is crucial that you review the available disk space on your nodes
  ...
  - For additional information, refer to the [UI Server Side
    Pagination - Disk Space
...
- Rancher now has an enablement option called
  ...
  for API Audit Logs for a Rancher installation.

  - In Rancher versions 2.11.x and earlier, only the `AUDIT_LEVEL` could
    be set and the default log level (`0`) would disable the audit log.
    In Rancher versions 2.12.x and later, the default log level (`0`)
    now only contains the log request and response metadata, and can be
    set when configuring `AUDIT_LOG_ENABLED`. If installing or upgrading
    via Helm you can enable the API Audit Logs and specify the log level
    by applying the following setting to your Helm command:
...
- CLI - [v2.13.2](https://github.com/rancher/cli/releases/tag/v2.13.2)
