...
:release-version: v2.14.5
:rn-component-version: v2.14
:previous-release-version: v2.14.4
...
* Fixed a security vulnerability in Fleet where cluster labels supplied by an agent during agent-initiated cluster registration were applied to the resulting `Cluster` object, including reserved `management.cattle.io/` labels used for bundle targeting. In deployments where multiple trust domains share a Fleet workspace namespace, this allowed a registering cluster to match another tenant's `Bundle` or `GitRepo` targets and receive that tenant's manifests and resolved values Secrets. Fleet now discards reserved labels supplied at agent-initiated registration. Standalone Rancher deployments are not affected in their default configuration, as Rancher uses manager-initiated registration. For more information, refer to https://github.com/rancher/fleet/security/advisories/GHSA-h9p5-fp5h-qpqr[CVE-2026-93538].
