...
:page-community-path: integrations-in-rancher/compliance-scans/custom-benchmark.adoc 
:page-product-path: security/compliance-scans/custom-benchmark.adoc
:experimental:
ifeval::["{build-type}" != "community"]
:xref-configuration-reference: xref:security/compliance-scans/configuration-reference.adoc#_xccdf_metadata_for_custom_benchmarks
endif::[]
ifeval::["{build-type}" == "community"]
:xref-configuration-reference: xref:integrations-in-rancher/compliance-scans/configuration-reference.adoc#_xccdf_metadata_for_custom_benchmarks
endif::[]
...
. (Optional) To enrich XCCDF report output with benchmark metadata, create a `metadata.yaml` file in the same directory and include it in the ConfigMap. For example:
+
[,yaml]
----
...
----
+
Refer to {xref-configuration-reference}[Configuration Reference] for all supported fields.
