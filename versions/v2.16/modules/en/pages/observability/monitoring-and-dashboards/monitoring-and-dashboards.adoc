...
:page-community-path: integrations-in-rancher/monitoring-and-alerting/index.adoc 
:page-product-path: observability/monitoring-and-dashboards/monitoring-and-dashboards.adoc
...
[IMPORTANT]
====
By default, the rancher-monitoring-dashboards chart does not enable metric scraping for `kubeEtcd`, `kubeControllerManager`, `kubeScheduler`, or `kubeProxy`. To export metrics from these components, you must configure https://github.com/prometheus-community/PushProx[PushProx].
====
...
This section guides you through setting up your Prometheus stack and installing the `rancher-monitoring-dashboards` chart using the xref:integrations/fleet/appco.adoc[SUSE Application Collection].
...
[,shell]
----
helm install kube-prometheus-stack oci://dp.apps.rancher.io/charts/prometheus-operator \
  --namespace cattle-monitoring-system \
  --create-namespace \
  --debug \
  --wait \
...
. Locate your cluster and click *Explore*.
. In the left navigation menu, select *Apps > Charts*.
. Locate and select the latest *rancher-monitoring-dashboards* chart.
. Click *Install*.
. In the *Namespace* dropdown, select *cattle-monitoring-system*.
. Click *Next*.
. The default `values.yaml` sets the upstream services name to the default value provided by the upstream `kube-prometheus-stack` and NGINX image. When using values provided by the SUSE Application Collection, you must modify the names of the services and the images. The following is an example with modified chart values:
+
.Click to expand
[%collapsible]
======
...
alertmanagerProxy:
  enabled: true
  upstreamPort: 9093
  upstreamService: kube-prometheus-stack-prom-alertmanager
commonLabels: {}
dashboardArtifacts:
  annotations: {}
...
monitoringProxy:
  ...
  image:
    pullPolicy: IfNotPresent
    repository: dp.apps.rancher.io/containers/nginx
    tag: 1.31.5
  nodeSelector: {}
  replicas: 1
  resources:
...
prometheusProxy:
  enabled: true
  upstreamPort: 9090
  upstreamService: kube-prometheus-stack-prom-prometheus
rancherMonitoring:
  enabled: true
  prometheusServiceAccount: kube-prometheus-stack-prom-prometheus
  selector: {}
----
======
...
ifeval::["{build-type}" != "community"]
[#_deploying_prometheus_federator_with_suse_application_collection]
=== Deploying Prometheus Federator with SUSE Application Collection

[IMPORTANT]
====
* The `chartSource.repo` URL must point strictly to the repository root directory, do not append `/index.yaml` to the URL: +https://raw.githubusercontent.com/rancher/ob-team-charts/refs/heads/main+.

* Ensure you include the following in your `values.yaml` file to use the SUSE Application Collection images:
+
[,yaml]
----
global:
...
If installing using the UI ensure all required fields under the **Project Monitoring Chart** and **Grafana Chart** tabs are filled respective to your `values.yaml` configuration, note the example UI view below. This changes the **image**, not the chart. At the moment it is not possible to choose what version of the Grafana chart is installed.

image::UI-prometheus-example.png[UI Prometheus Charts Example]

To customize the Prometheus, Grafana, and Alertmanager images, you need to change the following values in your configuration when deploying Prometheus Federator, for example Grafana:
...
helmProjectOperator:
  ...
  chartSource:
    ...
    repo: https://raw.githubusercontent.com/rancher/ob-team-charts/refs/heads/main
    version: 1.0.0
----
