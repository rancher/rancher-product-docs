...
:page-community-path: how-to-guides/advanced-user-guides/capi-infrastructure-providers.adoc
:page-product-path: cluster-deployment/configuration/capi-infrastructure-providers.adoc 
...
Current support for installable UI extensions is limited to CAPA.
...
. **Cluster Options**:
   ...
   ** **machineSelectorConfig**: Currently not supported in the UI form.
   * **Machine Pools**:
   ** Add one or more machine pools and assign node roles (**Control Plane**, **etcd**, **Worker**).
   ** Configure the machine pool parameters (e.g. subnet, IAM instance profile name, machine image).
