...
:page-community-path: how-to-guides/new-user-guides/kubernetes-clusters-in-rancher-setup/set-up-clusters-from-hosted-kubernetes-providers/aks.adoc 
:page-product-path: cluster-deployment/hosted-kubernetes/aks/aks.adoc
:experimental:
...
To interact with Azure APIs, an AKS cluster requires an Microsoft Entra ID service principal. The service principal is needed to dynamically create and manage other Azure resources, and it provides credentials for your cluster to communicate with AKS. For more information about the service principal, refer to the https://docs.microsoft.com/en-us/azure/aks/kubernetes-service-principal[AKS documentation].
...
. Enter a name for your service principal (application).
. (Optional) Choose which accounts can use the service principal.
. Click *Register*. You should now see the name of your service principal under menu:Microsoft Entra ID[App registrations].
...
*Result:* You have created a service principal and you should be able to see it in your Azure Portal listed in the *Microsoft Entra ID* section under *App registrations > All Applications*. You still need to give the service principal access to AKS.

Follow these instructions to give role-based access to your service principal.
...
. On the *Add role assignment* tab, click *Add*.
. On the *Role* tab, select a role that will have access to AKS. For example, you can use the *Contributor* role, which has permission to manage everything except for giving access to other users.
