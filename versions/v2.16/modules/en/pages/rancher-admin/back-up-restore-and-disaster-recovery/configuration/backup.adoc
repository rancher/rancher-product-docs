...
Beginning with the introduction of {xref-tokens}[Public API tokens] in Rancher 2.13, the resource set `rancher-resource-set-basic` is no longer suitable for production environments due to the use of Kubernetes Secrets as the token's backing store. In other words, public API tokens are neither saved nor restored when using `rancher-resource-set-basic`. With Rancher 2.15 switching its {xref-kubeconfig}[Kubeconfig] resources to use public API tokens, this impact is now visible to users.
