...
:page-community-path: reference-guides/backup-restore-configuration/backup-configuration.adoc 
:page-product-path: rancher-admin/back-up-restore-and-disaster-recovery/configuration/backup.adoc
...
:xref-kubeconfig: xref:api/workflows/kubeconfigs.adoc
:xref-tokens: xref:api/workflows/tokens.adoc
endif::[]
ifeval::["{build-type}" == "community"]
...
Beginning with the introduction of {xref-tokens}[Public API tokens] in Rancher 2.13 the resource set `rancher-resource-set-basic` is not suitable for use in production environments anymore, due to the use of Kubernetes secrets as the token's backing store. In other words, public API tokens are neither saved nor restored when using `rancher-resource-set-basic`. With Rancher 2.15's switch of its {xref-kubeconfig}[Kubeconfig] resources to make use of public API tokens this is now visible to users.

Given this it is recommended to use the resource set `rancher-resource-set-full` in production.
====
