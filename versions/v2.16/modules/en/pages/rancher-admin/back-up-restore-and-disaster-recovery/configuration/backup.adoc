...
Use `rancher-resource-set-full` in production environments.

Starting in Rancher 2.13,  Kubernetes secrets serve as the backing store for {xref-tokens}[Public API tokens].  As `rancher-resource-set-basic` excludes Kubernetes secrets, this resource set does not save or restore public API tokens. Because Rancher 2.15 updates {xref-kubeconfig}[Kubeconfig] resources to rely on public API tokens, `rancher-resource-set-basic` is no longer suitable for production environments.
====
