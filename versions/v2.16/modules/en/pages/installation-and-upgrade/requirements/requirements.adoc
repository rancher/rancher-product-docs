...
:page-community-path: getting-started/installation-and-upgrade/installation-requirements/index.adoc 
:page-product-path: installation-and-upgrade/requirements/requirements.adoc
...
[#_operating_systems_and_cpu_architectures]
...
** *Supported Architectures*: `x86_64`, `ARM64`, or mixed-architecture node pools.

[#_container_runtime_requirements_for_rancher_management_server]
...
* *RKE2 and K3s (Recommended)*: Use an embedded `containerd` runtime. No host-level container runtime installation is required prior to deployment.
* *Hosted Kubernetes (EKS, AKS, GKE)*: Uses the cloud provider's default CRI-compliant container runtime (such as `containerd` or `CRI-O`).

[#_multi_architecture_image_handling]
...
[#_node_time_synchronization_ntp]
...
[#_networking_and_firewall_requirements]
...
* *Alternative firewalld Configuration*: If your security policy requires `firewalld` to remain active, you must configure a dedicated `firewalld` zone with an `ACCEPT` policy targeting the Pod CIDR network and internal node communication.
