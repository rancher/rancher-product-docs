...
:page-community-path: how-to-guides/new-user-guides/helm-charts-in-rancher/index.adoc 
:page-product-path: cluster-admin/helm-charts-in-rancher/helm-charts-in-rancher.adoc
:experimental:
...
Rancher uses a catalog-like system to import bundles of charts from repositories and then uses those charts to either deploy custom Kubernetes applications or Rancher's tools such as Monitoring. Rancher tools come as pre-loaded repositories which deploy as standalone Helm charts. Any additional repositories are only added to the current cluster.
...
By default, Rancher syncs each Helm repository every hour (3600 seconds). You can configure the frequency using the `refreshInterval` field of the `ClusterRepo` custom resource definition (CRD).

To modify the refresh interval of a chart repository:

. Select *☰ > Cluster Management*.
. Locate the cluster containing the repository you want to modify, and click *Explore*.
. In the left navigation menu, select *Apps > Repositories*.
. Locate the repository, and select *⋮ > Edit Config*.
. Select *Enable Refresh Interval*.
. In the *Refresh Interval* field, enter a number.
. From the *Unit* dropdown menu, select a time unit (*secs*, *mins*, *hours*, or *days*).
. Click *Save*.

*Result:* Rancher syncs the repository according to the configured interval.
...
[NOTE]
====
If the cluster uses an authenticated registry, Rancher automatically injects the required pull secrets into the Helm values for charts from the built-in Rancher catalog. This behavior does not apply to charts with custom pull secrets specified during installation or charts from custom repositories. For more information, see {xref-authenticated-private-registries}[Authenticated Private Registries].
====
