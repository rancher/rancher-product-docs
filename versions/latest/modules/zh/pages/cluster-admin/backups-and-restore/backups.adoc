...
默认情况下，xref:../launch-kubernetes-with-rancher/launch-kubernetes-with-rancher.adoc[Rancher 启动的 Kubernetes 集群]会配置为创建定期快照（保存到本地磁盘）。为防止本地磁盘故障，建议使用 <<_s3_备份目标,S3 目标>>或复制磁盘上的路径。
...
默认情况下，xref:../launch-kubernetes-with-rancher/launch-kubernetes-with-rancher.adoc[Rancher 启动的 Kubernetes 集群]从凌晨 12 点开始每 5 小时创建一次定期快照（保存到本地磁盘）。为了防止本地磁盘故障，建议使用 <<_s3_备份目标,S3 目标>>或复制磁盘上的路径。
...
== 快照备份目标 Rancher 支持两种不同的备份目标： - <<_本地备份目标,本地目标>> - <<_s3_备份目标,S3 目标>>
