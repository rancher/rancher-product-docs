...
* xref:manage-templates.adoc[创建和修改模板]
...
你可以参见此xref:template-example-yaml.adoc[模板的示例 YAML 文件]作为参考。
...
如果将 RKE 模板定义为 YAML 文件，则可以修改此xref:template-example-yaml.adoc[示例 RKE 模板 YAML]。RKE 模板中的 YAML 使用了 Rancher 在创建 RKE 集群时使用的相同自定义设置。但由于 YAML 要在 Rancher 配置的集群中使用，因此需要将 RKE 模板自定义项嵌套在 YAML 中的 `rancher_kubernetes_engine_config` 参数下。
