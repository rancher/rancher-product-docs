...
使用 backup-restore-operator 执行恢复后，Fleet 中会出现一个已知问题：用于 clientSecretName 和 helmSecretName 的密文不包含在 Fleet 的 Git 仓库中。请参见link:../../integrations/fleet/overview.adoc#故障排除[此处]获得解决方法。
====
...
. 点击 *Rancher 备份*。
. 单击**安装**。
. 可选：配置默认存储位置。如需获取帮助，请参见xref:configuration/storage.adoc[配置]。
. 单击**安装**。
...
备份是通过创建 Backup 自定义资源实现的。如需查看教程，请参见xref:back-up.adoc[本页面]。

== 还原 Rancher

还原是通过创建 Restore 自定义资源实现的。如需查看教程，请参见xref:restore.adoc[本页面]。

== 将 Rancher 迁移到新集群

你可以按照xref:migrate-to-a-new-cluster.adoc[这些步骤]执行迁移。
...
如需了解各个选项的配置，请参见xref:configuration/storage.adoc[本页面]。
