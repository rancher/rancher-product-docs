...
如果系统无法直接访问互联网，请参见xref:../other-installation-methods/air-gapped/air-gapped.adoc[离线环境：Kubernetes 安装]。
...
要为大规模 Rancher 安装优化 etcd 集群，请参见 xref:../best-practices/tuning-etcd-for-large-installs.adoc[etcd 设置指南]。

有关 Dockershim 支持的详情，请参见xref:../requirements/dockershim.adoc[此页面]。
====
...
参见xref:../troubleshooting/troubleshooting.adoc[故障排除]页面。
