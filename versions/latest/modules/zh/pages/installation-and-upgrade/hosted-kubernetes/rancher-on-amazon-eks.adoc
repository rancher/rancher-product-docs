...
本文介绍了如何在 Amazon EKS 集群上安装 Rancher。你也可以xref:../quick-start/deploy-rancher/aws-marketplace.adoc[通过 AWS Marketplace 安装 Rancher]。

如果你已经有一个 EKS Kubernetes 集群，请直接跳转到<<_5_安装_ingress,安装 Ingress>>这个步骤。然后按照xref:../install-rancher.adoc#_安装_rancher_helm_chart[此处]的步骤安装 Rancher Helm Chart。
...
按照xref:../install-rancher.adoc#_安装_rancher_helm_chart[本页]的说明安装 Rancher Helm Chart。任何 Kubernetes 发行版上安装的 Rancher 的 Helm 说明都是一样的。
...
请参阅xref:../install-rancher.adoc#_5_根据你选择的证书选项通过_helm_安装_rancher[Helm 安装命令]了解你的证书选项。
