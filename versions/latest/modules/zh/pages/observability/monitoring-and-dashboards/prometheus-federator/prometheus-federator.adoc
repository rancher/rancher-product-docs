...
. RBAC 将自动分配到项目 Release 命名空间中，从而允许用户查看 Prometheus、Alertmanager 以及已部署的 Project Monitoring Stack 的 Grafana UI（基于在项目 Registration 命名空间上针对https://kubernetes.io/docs/reference/access-authn-authz/rbac/#user-facing-roles[面向用户的默认 Kubernetes 角色]定义的 RBAC）。有关详细信息，请参阅xref:observability/monitoring-and-dashboards/prometheus-federator/rbac.adoc[配置 RBAC]。
