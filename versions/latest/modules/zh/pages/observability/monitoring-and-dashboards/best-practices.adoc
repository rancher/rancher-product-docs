...
除了通过监控工作负载来检测性能、可用性或可扩展性之外，你还应该监控集群和运行在集群中的工作负载，来发现潜在的安全问题。A good starting point is to frequently run and alert on xref:security/compliance-scans/how-to.adoc[Compliance Scans] which check if the cluster is configured according to security best practices.
