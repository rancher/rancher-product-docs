...
升级到 v1.25 后，添加必要的 Rancher 命名空间豁免。有关详细信息，请参阅 xref:../psact.adoc#_豁免必须的_rancher_命名空间[Pod 安全准入 (PSA) 配置模板]。
====
...
. 将 xref:./rancher-psact.yaml[`rancher-psact.yaml`] 保存到 `/etc/rancher/rke2` 中。
. 编辑 RKE2 配置文件：
...
遵循 K3s https://docs.k3s.io/known-issues#hardened-125[将加固集群从 v1.24.x 升级到 v1.25.x]的官方说明，但使用xref:./rancher-psact.yaml[自定义]Rancher PSA 配置模板，而不是 K3s 官方网站上提供的配置。
--
======
