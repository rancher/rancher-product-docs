...
一些测试会被标记为"`不适用`"。由于 Rancher 配置 RKE 集群的方式，这些测试不会在任何 CIS 扫描中运行。有关如何审核测试结果，以及为什么某些测试会被标记为不适用，请参阅 Rancher 的 Kubernetes 对应版本的link:../security-overview.adoc#CIS-Benchmark-和自我评估[自测指南]。
...
要通过 "`Hardened`" 配置文件，你需要遵从link:../security-overview.adoc#Rancher-加固指南[强化指南]并使用强化指南中定义的 `cluster.yml` 来配置一个强化集群。
