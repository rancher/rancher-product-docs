...
[CAUTION]
====
Pod 安全策略仅在 Kubernetes v1.24 之前可用。xref:../psa-pss.adoc[Pod 安全标准] 是内置的替代方案。
====
...
你必须先在集群级别启用 PSP，然后才能将它们分配给项目。这可以通过xref:../../reference-guides/cluster-configuration/cluster-configuration.adoc[编辑集群]来配置。
