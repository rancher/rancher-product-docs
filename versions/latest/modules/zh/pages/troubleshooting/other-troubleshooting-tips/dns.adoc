...
在运行 DNS 检查之前，请检查集群的link:../../cluster-deployment/configuration/rke1.adoc#默认-dns-提供商[默认 DNS 提供商]，并确保link:networking.adoc#检查覆盖网络是否正常运行[覆盖网络正常运行]，因为这也可能导致 DNS 解析（部分）失败。
...
请参阅link:../../cluster-deployment/configuration/rke1.adoc#使用-yaml-编辑集群[使用 YAML 编辑集群]了解如何应用此修改。集群配置完成后，你必须删除 kube-dns pod 以激活 pod 中的新设置：
