...
仔细检查你的（主机）防火墙中是否打开了所有link:../../cluster-deployment/node-requirements.adoc#网络要求[必需的端口]。其他所需的端口都使用 TCP，而覆盖网络使用 UDP。
...
----
+
如果你在输出中看到错误，则说明两台主机上的 Pod 路由存在问题。在上面的输出中，节点 `wk2` 在覆盖网络上没有连接。原因可能是没有为 `wk2` 打开覆盖网络的link:../../cluster-deployment/node-requirements.adoc#网络要求[必需端口]。
