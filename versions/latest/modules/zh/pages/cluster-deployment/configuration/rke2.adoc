...
* <<_集群配置文件参考,集群配置文件>>：高级用户可以创建一个 RKE2 配置文件，而不是使用 Rancher UI 来为集群选择 Kubernetes 选项。配置文件让你能设置更多可用于 RKE2 的其他https://docs.rke2.io/install/configuration[安装选项]。
...
* xref:../infra-providers/azure/machine-configuration.adoc[Azure]
* xref:../infra-providers/digitalocean/machine-configuration.adoc[DigitalOcean]
* xref:../infra-providers/aws/machine-configuration.adoc[EC2]
...
所有 CNI 网络插件都支持https://docs.rke2.io/install/network_options#dual-stack-configuration[双栈]网络。要在双栈模式下配置 RKE2，请为你的<<_集群_cidr,集群 CIDR>> 和/或 <<service-cidr,Service CIDR>> 设置有效的 IPv4/IPv6 CIDR。
...
如果你要使用的云提供商未作为选项列出，你需要使用<<_集群配置文件参考,配置文件选项>>来配置云提供商。请参考https://rancher.com/docs/rke/latest/en/config-options/cloud-providers/[本文档]来了解如何配置云提供商。
====
...
为 xref:../about-rancher-agents.adoc[Rancher agent] 设置环境变量的选项。你可以使用键值对设置环境变量。有关详细信息，请参阅 https://docs.rke2.io/reference/linux_agent_config[RKE2 文档]。
