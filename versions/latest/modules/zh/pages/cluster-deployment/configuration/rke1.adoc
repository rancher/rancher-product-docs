...
Rancher 安装 Kubernetes 时，它使用 xref:../launch-kubernetes-with-rancher.adoc[RKE] 或 https://docs.rke2.io/[RKE2] 作为 Kubernetes 发行版。
...
在 xref:../launch-kubernetes-with-rancher.adoc[RKE 启动的集群]中，你可以编辑任何后续剩余的选项。
...
你可以配置 xref:../set-up-cloud-providers/set-up-cloud-providers.adoc[Kubernetes 云提供商]。如果你想在 Kubernetes 中使用动态配置的xref:../../cluster-admin/manage-clusters/persistent-storage/manage-persistent-storage.adoc[卷和存储]，你通常需要选择特定的云提供商。例如，如果你想使用 Amazon EBS，则需要选择 `aws` 云提供商。
...
有关使用 Rancher UI 在 RKE 集群中设置节点池的信息，请参阅xref:../infra-providers/infra-providers.adoc[此页面]。
...
为 xref:../about-rancher-agents.adoc[rancher agent] 设置环境变量的选项。你可以使用键值对设置环境变量。如果 Rancher Agent 需要使用代理与 Rancher Server 通信，则可以使用 Agent 环境变量设置 `HTTP_PROXY`，`HTTPS_PROXY` 和 `NO_PROXY` 环境变量。
