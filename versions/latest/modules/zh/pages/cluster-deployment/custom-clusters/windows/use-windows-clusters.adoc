...
在现有节点上使用 Rancher 配置集群时，你需要在每个节点上安装 xref:../rancher-agent-options.adoc[Rancher Agent] 来将节点添加到集群中。在 Rancher UI 中创建或编辑集群时，你会看到一个**自定义节点运行命令**，你可以在每台服务器上运行该命令，从而将服务器添加到集群中。
