...
使用 Rancher，你可以基于link:infra-providers/infra-providers.adoc#节点模板[节点模板]创建节点池。此模板定义了要在云提供商中启动的节点的参数。

使用由基础设施提供商托管的节点的一个好处是，如果一个节点与集群失去连接，Rancher 可以自动替换它，从而维护集群配置。

Rancher UI 中状态为 Active 的link:infra-providers/infra-providers.adoc#主机驱动[主机驱动]决定了可用于创建节点模板的云提供商。
