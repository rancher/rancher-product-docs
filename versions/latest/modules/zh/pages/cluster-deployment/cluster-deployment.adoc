...
详情请参阅xref:hosted-kubernetes/hosted-kubernetes.adoc[托管 Kubernetes 集群]部分。
...
有关详细信息，请参阅 xref:launch-kubernetes-with-rancher.adoc[RKE 集群]部分。
...
如需更多信息，请参阅xref:infra-providers/infra-providers.adoc[基础设施提供商托管的节点]部分。

=== 在现有自定义节点上启动 Kubernetes

在设置这种类型的集群时，Rancher 会在现有的xref:custom-clusters/custom-clusters.adoc[自定义节点]上安装 Kubernetes，从而创建一个自定义集群。
