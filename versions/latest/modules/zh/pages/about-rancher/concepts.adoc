...
Docker 是一个容器打包和运行时标准。开发者可以使用 Dockerfile 构建容器镜像，并从 Docker 镜像仓库分发容器镜像。link:https://hub.docker.com[Docker Hub] 是一个流行的公共镜像仓库。许多组织也建立了私有 Docker 镜像仓库。Docker 主要用于管理单个节点上的容器。
...
_Kubernetes 集群_ 是使用 https://kubernetes.io/[Kubernetes 容器编排系统]来部署、运维和伸缩 容器的集群，让你的组织实现应用自动化运维。
