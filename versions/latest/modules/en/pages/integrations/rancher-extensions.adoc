 ...
 ** *(Optional)* If the container image is private: xref:security/serets-hub.adoc[Create] a registry secret within the `cattle-ui-plugin-system` namespace. Enter the domain of the image address in the *Registry Domain Name* field.
. Click *☰*, then select *Extensions*, under *Configuration*.
. On the top right, click *⋮ > Manage Extension Catalogs*.
