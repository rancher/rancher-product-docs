...
====
You must consider how to handle Kubernetes API server availability and paused GitRepos when restoring a Rancher setup that is running Fleet workloads. See the https://documentation.suse.com/cloudnative/continuous-delivery/{fleet-docs-version}/en/gitrepo-add.html#_backing_up_and_restoring[{fleet-product-name} documentation] for more details.
:::
