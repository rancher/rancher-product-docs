...
If you already have an AKS Kubernetes cluster, skip to the step about <<5-install-an-ingress,installing an ingress.>> Then install the Rancher Helm chart following the instructions on link:../install-rancher.adoc#install-the-rancher-helm-chart[this page.]
...
* Your subscription has sufficient quota for at least 2 vCPUs. For details on Rancher server resource requirements, refer to xref:../requirements/installation-requirements.adoc[this section]
