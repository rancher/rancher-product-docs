...
When the nodes in your Kubernetes cluster are running and fulfill the xref:installation-and-upgrade/requirements/installation-requirements.adoc[node requirements,] you will use Helm to deploy Rancher onto Kubernetes. Helm uses Rancher's Helm chart to install a replica of Rancher on each node in the Kubernetes cluster. We recommend using a load balancer to direct traffic to each replica of Rancher in the cluster.
...
Before installing Rancher, make sure that your nodes fulfill all of the xref:installation-and-upgrade/requirements/installation-requirements.adoc[installation requirements.]
...
Refer to the xref:[docs about options for Docker installs] for details about other configurations including:

* With xref:#_api_audit_log[API auditing to record all transactions]
* With an xref:installation-and-upgrade/infrastructure-setup/configure-layer-7-nginx-load-balancer.adoc[external load balancer]
* With a xref:#_persistent_data[persistent data store]
