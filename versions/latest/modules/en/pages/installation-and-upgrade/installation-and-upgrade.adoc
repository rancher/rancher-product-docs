...
Note the `restrictedAdmin` Helm chart option available for *the Rancher Server*. When this option is set to true, the initial Rancher user has restricted access to the local Kubernetes cluster to prevent privilege escalation. For more information, see the section about the link:../rancher-admin/users/authn-and-authz/manage-role-based-access-control-rbac/global-permissions.adoc#restricted-admin[restricted-admin role.]
...
When the nodes in your Kubernetes cluster are running and fulfill the xref:requirements/installation-requirements.adoc[node requirements,] you will use Helm to deploy Rancher onto Kubernetes. Helm uses Rancher's Helm chart to install a replica of Rancher on each node in the Kubernetes cluster. We recommend using a load balancer to direct traffic to each replica of Rancher in the cluster.
...
Before installing Rancher, make sure that your nodes fulfill all of the xref:requirements/installation-requirements.adoc[installation requirements.]
