 ...
 ** Security group: `K8sMasterSg`. More info at xref:installation-and-upgrade/requirements/port-requirements.adoc#_downstream_kubernetes_cluster_nodes[RKE2 ports (custom nodes tab)]
 ** Tags:
...
* Security group: `K8sWorkerSg` More info at xref:installation-and-upgrade/requirements/port-requirements.adoc#_downstream_kubernetes_cluster_nodes[RKE2 ports (custom nodes tab)].
* Tags:
 ** `kubernetes.io/cluster/<clusterID>: owned`
...
More info is at xref:cluster-deployment/set-up-cloud-providers/amazon.adoc[RKE2 clusters on AWS] and https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/cloudprovider/aws/README.md[Cluster Autoscaler on AWS.]
