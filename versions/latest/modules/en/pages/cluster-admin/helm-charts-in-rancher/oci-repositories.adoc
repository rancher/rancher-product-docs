...
Rancher supports BasicAuth for OCI registries. You must create a https://kubernetes.io/docs/concepts/configuration/secret/#basic-authentication-secret[*BasicAuth* Kubernetes secret]. You can also xref:../../security/serets-hub.adoc[create the secret through the Rancher UI].
...
* To enhance logging information, link:../../observability/logging/troubleshooting.adoc#kubernetes-install[enable the debug option] while deploying Rancher.
