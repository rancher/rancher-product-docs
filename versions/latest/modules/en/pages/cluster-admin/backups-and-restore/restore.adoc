...
. After the single nodes is up and running, Rancher recommends adding additional etcd nodes to your cluster. If you have a xref:cluster-deployment/custom-clusters/custom-cluster.adoc[custom cluster] and you want to reuse an old node, you are required to xref:manage-clusters/clean-cluster-nodes.adoc#cleaning-up-nodes[clean up the nodes] before attempting to add them back into a cluster.
