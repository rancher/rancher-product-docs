...
. Click *Upgrade* to edit the base components via changes to the values.yaml or add an link:configuration/configuration.adoc#overlay-file[overlay file]. For more information about editing the overlay file, see <<editing-the-overlay-file,this section.>>
