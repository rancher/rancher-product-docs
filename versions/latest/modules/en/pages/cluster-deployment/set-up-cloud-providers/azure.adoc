...
You can also xref:../migrate-to-an-out-of-tree-cloud-provider/azure.adoc[migrate from an in-tree to an out-of-tree Azure cloud provider] on Kubernetes 1.29 and earlier. All existing clusters must migrate prior to upgrading to v1.30 in order to stay functional.
