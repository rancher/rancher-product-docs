...
. Install the `cloud-provider-azure` chart after the cluster finishes provisioning. Note that the cluster is not successfully provisioned and nodes are still in an `uninitialized` state until you deploy the cloud controller manager. This can be done <<helm-chart-installation-from-cli,manually using CLI>>, or via <<helm-chart-installation-from-ui,Helm charts in UI>>.
...
. Create a `azure-cloud-config` secret with the required <<cloud-provider-configuration,cloud provider config>>.
+
[,shell]
----
