...
After cloud credentials are created, the user can start xref:../../rancher-admin/user/settings/manage-cloud-credentials.adoc[managing the cloud credentials that they created].

=== Node Drivers

If you don't find the node driver that you want to use, you can see if it is available in Rancher's built-in link:../../rancher-admin/global-configuration/provisioning-drivers/manage-node-drivers.adoc#activatingdeactivating-node-drivers[node drivers and activate it], or you can link:../../rancher-admin/global-configuration/provisioning-drivers/manage-node-drivers.adoc#adding-custom-node-drivers[add your own custom node driver].
...
For RKE2 cluster templates, please refer to link:../../cluster-admin/manage-clusters/cluster-templates.adoc#rke2-cluster-template[this page] for additional information.
====
...
In our xref:../production-checklist/recommended-cluster-architecture.adoc[recommended cluster architecture], we outline how many nodes of each role clusters should have:
