...
Creating a xref:cluster-deployment/infra-providers/infra-providers.adoc#node-templates[node template] for Azure will allow Rancher to provision new nodes in Azure. Node templates can be reused for other clusters.
...
. Enter a *Cluster Name*.
. Create a machine pool for each Kubernetes role. Refer to the xref:cluster-deployment/infra-providers/infra-providers.adoc#node-roles[best practices] for recommendations on role assignments and counts.
