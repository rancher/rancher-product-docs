...
如果在操作员级别未配置默认存储位置，或者备份文件存在于与配置为默认存储位置的S3存储桶不同的存储桶中，请选择此选项。在 *备份文件名* 字段中提供确切的文件名。请参阅 <<_getting_the_backup_filename_from_s3,本节>>，了解如何从 S3 获取确切的备份文件名。填写S3兼容对象存储的所有详细信息。其字段与 `backup.StorageLocation` 中的配置的字段完全相同  {xref-rancher-backup-config}#_storage_location[备份自定义资源。]
