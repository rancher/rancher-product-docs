...
* *No bundled runtime:* Operates without depending on Rancher-shipped `kube-prometheus-stack` images or components. It installs a dashboard-only version of `rancher-monitoring`.
...
*Q*: How does this change affect project monitoring and `prometheus-federator`? +

*A*: Project monitoring workflows are undergoing updates to support:

  * Updating `prometheus-federator` to use external chart sources instead of embedded charts.
  * Updating `rancher-project-monitoring` to use the cluster's existing Grafana and Prometheus instances instead of deploying separate instances.
