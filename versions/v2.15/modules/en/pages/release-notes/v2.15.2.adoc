...
* *Rancher v2.15.1:*
** Google Cloud has deprecated the "No channel" option for GKE clusters. Automatic out-of-band upgrades can cause state drift in Rancher, which may block cluster edits, break node pool updates, or cause provisioning failures. An upcoming out-of-band GKE chart release will assign new clusters to a release channel by default. See https://github.com/rancher/rancher/issues/56879[#56879].
