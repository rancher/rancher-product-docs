...
* There is a known issue regarding v2prov clusters with etcd snapshots in s3 that cannot be deleted. A change was made to wait for all cluster-indexed resources to be deleted before deleting the cluster object, but whereas local snapshots are machine indexed, s3 snapshots belong to the cluster object. This results in clusters hanging forever, and while the related underlying infrastructure will be deleted, these clusters appear stuck. A workaround can be performed by deleting all left over s3 `etcdsnapshots.rke.cattle.io` objects belonging to the cluster that are present within the local cluster. See https://github.com/rancher/rancher/issues/57506[#57506] and the https://github.com/rancher/rancher/issues/57506#issuecomment-5784025617[issue comment] for more information.
