...
[NOTE]
====
If the cluster is configured to use an authenticated registry, Rancher automatically injects the required pull secrets into the Helm values of charts from the built-in Rancher catalog. This does not apply to charts which defined custom pull secrets during installation, or charts from custom repositories. For more information, see {xref-authenticated-private-registries}[Authenticated Private Registries].
====
