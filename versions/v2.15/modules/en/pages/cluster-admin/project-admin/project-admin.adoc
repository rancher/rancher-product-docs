...
:page-community-path: how-to-guides/advanced-user-guides/manage-projects/index.adoc 
:page-product-path: cluster-admin/project-admin/project-admin.adoc
...
* {xref-add-users-to-projects}[][Assign users access to a group of namespaces]
* Assign users {xref-cluster-project-roles}#_project_roles[specific roles in a project]. A role can be owner, member, read-only, or {xref-custom-roles}[custom]
* {xref-project-resource-quotas}[Set resource quotas]
* {xref-namespaces}[Manage namespaces]
* {xref-rancher-project-tools}[Configure tools]

[#_authorization]
== Authorization

Non-administrative users are only authorized for project access after an {xref-global-permissions}[administrator], {xref-cluster-project-roles}#_cluster_roles[cluster owner or member], or {xref-cluster-project-roles}#_project_roles[project owner] adds them to the project's *Members* tab.
