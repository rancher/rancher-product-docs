...
:page-community-path: how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/authentication-config/configure-azure-ad.adoc 
:page-product-path: rancher-admin/users/authn-and-authz/configure-azure-ad.adoc
...
. (Optional) Filter group memberships to reduce log volume and token overhead (Rancher v2.9.0+). See <<_filtering_users_by_azure_ad_auth_group_memberships,Filtering Users by Microsoft Entra ID Auth Group Memberships>> for details.
. Click **Enable**.
...
. Click the notification banner prompting an update for Microsoft Entra ID authentication.
+
