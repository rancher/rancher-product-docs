...
:page-community-path: how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/manage-role-based-access-control-rbac/global-permissions.adoc 
:page-product-path: rancher-admin/users/authn-and-authz/manage-role-based-access-control-rbac/global-permissions.adoc
:experimental:
ifeval::["{build-type}" != "community"]
:xref-global-resources: xref:rancher-admin/users/authn-and-authz/manage-role-based-access-control-rbac/global-resources.adoc
...
ifeval::["{build-type}" == "community"]
:xref-global-resources: xref:how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/manage-role-based-access-control-rbac/global-resources.adoc
...
See the list {xref-global-resources}[Global Resources] for relevant resources.
...
[#_globalrole_permissions_in_specific_namespaces]
==== GlobalRole Permissions in Specific Namespaces

GlobalRoles grant access to resources in specific namespaces using two fields: `namespacedRules` and `inheritedNamespacedRules`. 

The `namespacedRules` field provides access to namespaces within the management cluster. For example, when assigned to a user, the following `GlobalRole` allows that user to read secrets in the `my-secrets-namespace` namespace on the management cluster:
...
The `inheritedNamespacedRules` field applies permissions to namespaces across all downstream clusters. The following example shows a `GlobalRole` that grants full access to the `cattle-monitoring-system` namespace on every cluster:
...
If a cluster does not have the `cattle-monitoring-system` namespace, Rancher grants no permissions. If you later create the namespace on that cluster, Rancher automatically adds permissions for users bound to this `GlobalRole`.

Both fields are maps where keys are namespaces and values are arrays of `PolicyRules`. This structure allows you to specify multiple namespaces and multiple rules per namespace.

[NOTE]
====
These fields alone do not grant direct cluster access. You must also grant users permissions to access the cluster using `inheritedClusterRoles` or `ClusterRoleTemplateBindings`.
====

[#_globalrole_permissions_for_fleet_workspaces]
