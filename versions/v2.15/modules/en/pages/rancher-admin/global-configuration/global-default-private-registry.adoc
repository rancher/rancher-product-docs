...
. Optional: Configure Credentials for the Cluster-Level Registry
...
*Result:* The new cluster pulls images from the private registry. This can be changed later by editing the cluster configuration.
...
. Check the box next to **Enable cluster scoped container registry for Rancher system container images**.
. Enter the registry hostname.
. Optional: Select an optional image pull secret to authenticate with the registry
. Click **Create**.
