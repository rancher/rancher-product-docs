...
:page-community-path: how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/about-rke1-templates/manage-rke1-templates.adoc 
:page-product-path: rancher-admin/global-configuration/rke1-templates/manage-templates.adoc
:experimental:
...
You can create RKE templates if you have the *Create RKE Templates* permission, which can be {xref-rke1-creator-permissions}[given by an administrator.]
...
*Result:* An RKE template with one revision is configured. You can use this RKE template revision later when you {xref-launch-k8s-with-rancher}[provision a Rancher-launched cluster]. After a cluster is managed by an RKE template, it cannot be disconnected and the option to uncheck *Use an existing RKE Template and Revision* will be unavailable.
...
This section assumes that you already have a cluster that {xref-rke1-template-apply}[has an RKE template applied.]
