...
* `hide-local-auth-provider`: When enabled, hides the local auth provider from users if one or more external auth providers are configured and active. This prevents login, and creation or editing of `local-auth-provider users`, through the UI, API, and CLI. It does not revoke API keys that were already issued for `local-auth-provider` users, and users with the `manage-users` permission are unaffected. This flag is disabled by default.
+
[#hide-local-auth-provider-warning]
[WARNING]
====
Before enabling `hide-local-auth-provider`, make sure the default administrator has a valid, non-expiring-soon API key or `kubeconfig` access. If the external auth provider becomes unavailable while this flag is active, login through the local auth provider is also blocked in the UI, API, and CLI. Remember that API keys have an expiration time, so generate a new API key before the current one expires.

To recover, use `kubectl` to disable the `hide-local-auth-provider` flag, since login through the UI or API is not possible in that failure state. Alternatively, users with the `manage-users` permission bypass these restrictions and can still manage local auth provider users.
====
...
| GA
| v2.15.0
| See the <<hide-local-auth-provider-warning,warning>> above about the risk of being locked out of the UI, API, and CLI.
