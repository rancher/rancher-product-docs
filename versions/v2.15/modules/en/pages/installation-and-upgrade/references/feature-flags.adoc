...
[WARNING]
====
If the external auth provider becomes unavailable whilst this flag is active it will not be possible to login via UI, API or CLI. 

So when `hide-local-auth-provider` is enabled, make sure an administrator has a valid, non-expiring-soon API key or `kubeconfig` token for the local cluster. These can be used to recover the system by disabling the flag.
