...
[WARNING]
====
If the external auth provider becomes unavailable whilst this flag is active it will not be possible to log in via UI, API or CLI.

When `hide-local-auth-provider` is enabled, make sure an administrator has a valid, non-expiring-soon API key or `kubeconfig` token for the local cluster. These can be used to recover the system by disabling the flag.

To recover, use `kubectl` to disable the `hide-local-auth-provider` flag, since login through the UI or API is not possible in that failure state.
====
