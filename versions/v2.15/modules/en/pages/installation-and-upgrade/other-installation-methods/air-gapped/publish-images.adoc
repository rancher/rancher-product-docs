...
:page-community-path: getting-started/installation-and-upgrade/other-installation-methods/air-gapped-helm-cli-install/publish-images.adoc 
:page-product-path: installation-and-upgrade/other-installation-methods/air-gapped/publish-images.adoc
...
[NOTE]
====
`rancher-images.txt` may include OCI Helm chart references (entries containing `rancher/charts/`). Docker can pull and push these natively only when the daemon is configured to use the https://docs.docker.com/engine/storage/containerd/#enable-containerd-image-store-on-docker-engine[containerd image store snapshotter]. If your Docker daemon does not use the containerd snapshotter, install the https://helm.sh/docs/intro/install/[Helm CLI] (v3.8.0 or later) on your workstation. In that case, `rancher-save-images.sh` and `rancher-load-images.sh` automatically detect the missing snapshotter and fall back to `helm pull`/`helm push` for the OCI Helm charts, storing them in a `./rancher-oci-charts/` directory alongside `rancher-images.tar.gz`. If neither the containerd snapshotter nor the Helm CLI in a required version is available, the scripts exit with an error.
====
...
----
+
*Result:* Docker begins pulling the images used for an air-gap install. Be patient. This process takes a few minutes. If the image list contains OCI Helm charts and the Helm CLI fallback is used, the charts are saved to a `./rancher-oci-charts/` directory instead of the tarball.
...
The `rancher-images.txt` is expected to be on the workstation in the same directory that you are running the `rancher-load-images.sh` script. The `rancher-images.tar.gz` should also be in the same directory. If a `./rancher-oci-charts/` directory was created when saving the images, transfer it to this workstation as well and place it in the same directory before running `rancher-load-images.sh`. The `./rancher-oci-charts/` directory can be safely removed after the script finishes.
...
*Result:* Docker begins pulling the images used for an air-gap install. Be patient. This process takes a few minutes. When the process completes, your current directory will output a tarball named `rancher-images.tar.gz`. Check that the output is in the directory. If the image list contains OCI Helm charts and the Helm CLI fallback is used, the charts are saved to a `./rancher-oci-charts/` directory instead of the tarball.
...
The image list, `rancher-images.txt` or `rancher-windows-images.txt`, is expected to be on the workstation in the same directory that you are running the `rancher-load-images.sh` script. The `rancher-images.tar.gz` should also be in the same directory. If a `./rancher-oci-charts/` directory was created when saving the images, transfer it to this workstation as well and place it in the same directory before running `rancher-load-images.sh`. The `./rancher-oci-charts/` directory can be safely removed after the script finishes.
