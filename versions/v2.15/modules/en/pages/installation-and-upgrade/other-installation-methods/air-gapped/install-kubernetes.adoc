...
:page-community-path: getting-started/installation-and-upgrade/other-installation-methods/air-gapped-helm-cli-install/install-kubernetes.adoc 
:page-product-path: installation-and-upgrade/other-installation-methods/air-gapped/install-kubernetes.adoc
...
:xref-install-rancher-ha: xref:installation-and-upgrade/other-installation-methods/air-gapped/install-rancher-ha.adoc
endif::[]
ifeval::["{build-type}" == "community"]
...
:xref-install-rancher-ha: xref:getting-started/installation-and-upgrade/other-installation-methods/air-gapped-helm-cli-install/install-rancher-ha.adoc
endif::[]
...
+
--
In this guide, we are assuming you have created your nodes in your air-gapped environment and have a secure private container registry (or OCI-compliant registry) on your bastion server.
...
The `registries.yaml` file configures registry credentials at the node level for K3s and RKE2. If you install Rancher on another Kubernetes distribution, or cannot configure node-level registries, see {xref-install-rancher-ha}#_helm_chart_options_for_air_gap_installations[Helm Chart Options for Air Gap Installations] to configure credentials using the `system-default-regsitry` and `system-default-registry-pull-secrets` global settings instead.
