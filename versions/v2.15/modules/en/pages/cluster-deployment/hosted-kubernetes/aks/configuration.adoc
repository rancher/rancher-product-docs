...
:page-community-path: reference-guides/cluster-configuration/rancher-server-configuration/aks-cluster-configuration.adoc 
:page-product-path: cluster-deployment/hosted-kubernetes/aks/configuration.adoc
...
To get the Application (Client) ID:
...
. On the *Overview* page, locate and copy the value listed under *Application (client) ID*.
...
To get a new client secret:

. Sign in to the https://portal.azure.com[Azure Portal].
. Search for and select *Microsoft Entra ID* (formerly _Azure Active Directory_).
. In the left navigation menu, select *App registrations*.
. Click the name of your application.
. Under the *Manage* section in the left menu, select *Certificates & secrets*.
. Select the *Client secrets* tab, then click *+ New client secret*.
. Provide a description, choose an expiration period, and click *Add*.
