...
:page-community-path: how-to-guides/new-user-guides/kubernetes-clusters-in-rancher-setup/set-up-cloud-providers/azure.adoc 
:page-product-path: cluster-deployment/set-up-cloud-providers/azure.adoc
...
Visit the https://portal.azure.com[Azure portal], sign in, and go to *Microsoft Entra ID* (formerly _Azure Active Directory_). On the *Overview* page (or under *Properties*), copy the value listed as *Tenant ID*.
...
Visit the https://portal.azure.com[Azure portal], sign in, and follow the steps below to create an *App Registration* and obtain the corresponding *Azure Client ID* (`aadClientId`) and *Azure Client Secret* (`aadClientSecret`).
...
. Under *Supported account types*, choose who can use the application (the default, _Accounts in this organizational directory only_, is typical for internal integrations).
. Select *Register*.

Once registered, you will be directed to the application's *Overview* page. Copy the value listed under *Application (client) ID, this is your *Azure Client ID* (`aadClientId`).

==== Generate the Azure Client Secret

. In the left navigation menu under *Manage*, select *Certificates & secrets*.
. Select the *Client secrets* tab and click *+ New client secret*.
. Enter a *Description*, select an expiration period, and click *Add*.
. Copy the string in the *Value* column—this is your *Azure Client Secret* (`aadClientSecret`).

[IMPORTANT]
====
Copy the secret *Value* immediately after creation. It will only be displayed once and will be permanently masked (`••••••••••••••••`) after you navigate away or refresh the page. Do not copy the *Secret ID*.
====
