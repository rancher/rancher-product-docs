...
Because of this change to the controller, the default RKE1 behavior no longer sets `hostNetwork` to `true`. However, you must set `hostNetwork` to `true` for TCP- and UDP-based Services to work. To do so, link:../../../cluster-deployment/configuration/rke1.adoc#editing-clusters-with-yaml[edit] the cluster's YAML and follow the steps in the https://rke.docs.rancher.com/config-options/add-ons/ingress-controllers#configuring-network-options[official RKE1 doccumention].
====
