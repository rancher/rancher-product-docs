...
* *Known Issue*: clientSecretName and helmSecretName secrets for Fleet gitrepos are not included in the backup nor restore created by the link:../../rancher-admin/back-up-restore-and-disaster-recovery/back-up.adoc#1-install-the-rancher-backup-operator[backup-restore-operator]. We will update the community once a permanent solution is in place.
