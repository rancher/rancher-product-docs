...
Some tests are designated as "Not Applicable." These tests will not be run on any CIS scan because of the way that Rancher provisions RKE clusters. For information on how test results can be audited, and why some tests are designated to be not applicable, refer to Rancher's xref:../security-overview.adoc#_the_cis_benchmark_and_self_assessment[self-assessment guide] for the corresponding Kubernetes version.
...
In order to pass the "Hardened" profile, you will need to follow the steps on the xref:../security-overview.adoc#_rancher_hardening_guide[hardening guide] and use the `cluster.yml` defined in the hardening guide to provision a hardened cluster.
