...
. Save xref:security/hardening-guides/rancher-psact.yaml[`rancher-psact.yaml`] in `/etc/rancher/rke2`.
...
Follow the official K3s instructions on https://docs.k3s.io/known-issues#hardened-125[Upgrading Hardened Clusters from v1.24.x to v1.25.x], but use a xref:security/hardening-guides/rancher-psact.yaml[custom] Rancher PSA configuration template, instead of the configuration provided on the official K3s site.
--
======
