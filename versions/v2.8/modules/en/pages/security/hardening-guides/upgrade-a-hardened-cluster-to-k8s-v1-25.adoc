...
After you upgrade to v1.25, add the necessary Rancher namespace exemptions. See link:../psact.adoc#exempting-required-rancher-namespaces[Pod Security Admission (PSA) Configuration Templates] for more details.
====
...
. Save link:./rancher-psact.yaml[`rancher-psact.yaml`] in `/etc/rancher/rke2`.
...
Follow the official K3s instructions on https://docs.k3s.io/known-issues#hardened-125[Upgrading Hardened Clusters from v1.24.x to v1.25.x], but use a link:./rancher-psact.yaml[custom] Rancher PSA configuration template, instead of the configuration provided on the official K3s site.
--
======
