...
If you are upgrading cert-manager to the latest version from v1.5 or below, follow the xref:./resources/upgrade-cert-manager.adoc#_option_c_upgrade_cert_manager_from_versions_15_and_below[cert-manager upgrade docs] to learn how to upgrade cert-manager without needing to perform an uninstall or reinstall of Rancher. Otherwise, follow the <<_steps_to_upgrade_rancher,steps to upgrade Rancher>> below.
