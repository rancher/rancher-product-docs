...
This section describes how to install a Kubernetes cluster according to our link:../../../about-rancher/architecture/recommendations.adoc#environment-for-kubernetes-installations[best practices for the Rancher server environment.] This cluster should be dedicated to run only the Rancher server.
