...
This page covers installing Rancher on an Amazon EKS cluster.  You can also xref:../quick-start/deploy-rancher/aws-marketplace.adoc[install Rancher through the AWS Marketplace].

If you already have an EKS Kubernetes cluster, skip to the step about <<5-install-an-ingress,installing an ingress.>> Then install the Rancher Helm chart following the instructions on link:../install-rancher.adoc#install-the-rancher-helm-chart[this page.]
