...
.Important:
====
   As the certificate is signed by a private CA, it is important to ensure xref:../references/helm-chart-options.adoc#_common_options[`privateCA: true`] is set in the `values.yaml` file.
====
...
. Update the current Helm values in the `values.yaml` file:
 ...
 .. Adjust the `ingress.tls.source` field as necessary. Please xref:../references/helm-chart-options.adoc#_common_options[refer to the chart options] for more details. Here are some examples:
