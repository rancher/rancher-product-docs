...
Follow these steps to rotate an SSL certificate and private CA used by Rancher xref:../install-rancher.adoc[installed on a Kubernetes cluster], or migrate to an SSL certificate signed by a private CA.
...
.Important:
====
   As the certificate is signed by a private CA, it is important to ensure link:../references/helm-chart-options.adoc#common-options[`privateCA: true`] is set in the `values.yaml` file.
====
...
Using a Kubeconfig for each downstream cluster update the environment variable for the two agent deployments. If the xref:../../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.adoc[ACE] is enabled for the cluster, link:../../cluster-admin/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.adoc#authenticating-directly-with-a-downstream-cluster[the kubectl context can be adjusted] to connect directly to the downstream cluster.
...
Select 'Force Update' for the clusters within the link:../../integrations/fleet/overview.adoc#accessing-fleet-in-the-rancher-ui[Continuous Delivery] view of the Rancher UI to allow the fleet-agent in downstream clusters to successfully connect to Rancher.
...
. Update the current Helm values in the `values.yaml` file:
 ...
 .. Adjust the `ingress.tls.source` field as necessary. Please link:../references/helm-chart-options.adoc#common-options[refer to the chart options] for more details. Here are some examples:
