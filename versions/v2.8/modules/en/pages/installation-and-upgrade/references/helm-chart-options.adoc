...
For information on enabling experimental features, refer to xref:../../rancher-admin/experimental-features/experimental-features.adoc[this page.]
...
For details on installing Rancher with a private registry, see the xref:../other-installation-methods/air-gapped/air-gapped.adoc[air gap installation docs.]
...
If you are using a Private CA signed certificate (or if `agent-tls-mode` is set to `strict`), add `--set privateCA=true` and see xref:../resources/tls-secrets.adoc[Adding TLS Secrets - Using a Private CA Signed Certificate] to add the CA cert for Rancher.
====
