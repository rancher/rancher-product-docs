...
In this tutorial, you will learn one way to set up Linux nodes for the Rancher management server. These nodes will fulfill the node requirements for xref:../requirements/requirements.adoc[OS, Docker, hardware, and networking.]
...
* *Create security group:* We also recommend setting up a security group for the Rancher nodes that complies with the xref:../requirements/requirements.adoc#_port_requirements[port requirements for Rancher nodes.]
...
. In *Step 6: Configure Security Group,* select a security group that complies with the xref:../requirements/requirements.adoc#_port_requirements[port requirements] for Rancher nodes.
