...
This occurs when Rancher was configured with `agent-tls-mode` set to `strict`, but couldn't find cacerts in the `cacert` setting. To resolve the issue, set the `agent-tls-mode` to `system-store`, or upload the CA for Rancher as described in xref:../resources/tls-secrets.adoc[Adding TLS Secrets].
...
To resolve the issue, set the `agent-tls-mode` to `system-store` or upload the CA for Rancher as described in xref:../resources/tls-secrets.adoc[Adding TLS Secrets].
