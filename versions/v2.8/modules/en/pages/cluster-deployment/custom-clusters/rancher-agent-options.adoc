...
Rancher deploys an agent on each node to communicate with the node. This pages describes the options that can be passed to the agent. To use these options, you will need to xref:custom-clusters.adoc[create a cluster with custom nodes] and add the options to the generated `docker run` command when adding a node.

For an overview of how Rancher communicates with downstream clusters using node agents, refer to the link:../../about-rancher/architecture/communicating-with-downstream-clusters.adoc#3-node-agents[architecture section.]
