...
* Use dedicated nodes for each role. This ensures resource availability for the components needed for the specified role. It also strictly isolates network traffic between each of the roles according to the xref:../node-requirements.adoc#_networking_requirements[port requirements].
