...
* Use dedicated nodes for each role. This ensures resource availability for the components needed for the specified role. It also strictly isolates network traffic between each of the roles according to the link:../node-requirements.adoc#networking-requirements[port requirements].
