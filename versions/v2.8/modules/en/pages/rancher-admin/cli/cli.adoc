...
* An API Bearer Token, which is used to authenticate with Rancher. For more information about obtaining a Bearer Token, see xref:../users/settings/api-keys.adoc[Creating an API Key].
...
| `context`
| Switches between Rancher xref:../../cluster-admin/manage-clusters/projects-and-namespaces.adoc[projects]. For an example, see <<project-selection,Project Selection>>.
