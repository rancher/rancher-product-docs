...
Rancher uses LDAP to communicate with the Active Directory server. The authentication flow for Active Directory is therefore the same as for the xref:openldap/openldap.adoc[OpenLDAP authentication] integration.

[NOTE]
====

Before you start, please familiarise yourself with the concepts of link:authn-and-authz.adoc#external-authentication-configuration-and-principal-users[External Authentication Configuration and Principal Users].
====
