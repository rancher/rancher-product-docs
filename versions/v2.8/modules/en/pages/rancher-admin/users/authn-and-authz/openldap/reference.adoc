...
If you are in doubt about the correct values to enter in the user/group Search Base configuration fields, consult your LDAP administrator or refer to the section xref:rancher-admin/users/authn-and-authz/configure-active-directory.adoc#_annex_identify_search_base_and_schema_using_ldapsearch[Identify Search Base and Schema using ldapsearch] in the Active Directory authentication documentation.+++<figcaption>+++OpenLDAP Server Parameters+++</figcaption>+++
