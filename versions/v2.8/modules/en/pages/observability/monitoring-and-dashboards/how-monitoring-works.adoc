...
By editing custom YAML in the Alertmanager or Receiver configuration, you can also send alerts to multiple notification systems. For more information, see the section on configuring link:configuration/receivers.adoc#configuring-multiple-receivers[Receivers.]
...
Metrics are scraped differently based on the Kubernetes distribution. For help with terminology, refer <<_terminology,here>>. For details, see the table below:+++<figcaption>+++How Metrics are Exposed to Prometheus+++</figcaption>+++
