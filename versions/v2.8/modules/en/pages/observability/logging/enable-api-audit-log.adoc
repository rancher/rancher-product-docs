...
* xref:../../reference-guides/single-node-rancher-in-docker/advanced-options.adoc#_api_audit_log[Docker Install]
...
You can enable Rancher's built in log collection and shipping for the cluster to ship the audit and other services logs to a supported collection endpoint. See xref:logging.adoc[Rancher Tools - Logging] for details.
