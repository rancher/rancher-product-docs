...
Kubernetes Worker 需要打开 UDP 端口 `8472` (VXLAN)。有关详细信息，请参阅link:../cluster-deployment/node-requirements.adoc#网络要求[下游集群的端口要求]。
...
Kubernetes worker 需要打开 TCP 端口 `6783`（控制端口）、UDP 端口 `6783` 和 UDP 端口 `6784`（数据端口）。有关详细信息，请参阅link:../cluster-deployment/node-requirements.adoc#网络要求[下游集群的端口要求]。
...
如果使用 BGP，Kubernetes Worker 需要打开 TCP 端口 `179`，如果使用 VXLAN 封装，则需要打开 UDP 端口 `4789`。另外，使用 Typha 时需要 TCP 端口 `5473`。有关详细信息，请参阅link:../cluster-deployment/node-requirements.adoc#网络要求[下游集群的端口要求]。
...
如需了解如何为你的集群配置网络插件，请参阅xref:../cluster-deployment/configuration/rke1.adoc[集群选项]。有关更高级的配置选项，请参阅有关使用link:../cluster-deployment/configuration/rke1.adoc#rke-集群配置文件参考[配置文件]和https://rancher.com/docs/rke/latest/en/config-options/add-ons/network-plugins/[网络插件]选项来配置集群的说明。
