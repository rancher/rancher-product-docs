...
Rancher 建议启用 link:backups.adoc#配置定期快照[etcd 定期快照的功能]，但你也可以轻松创建link:backups.adoc#单次快照[一次性快照]。Rancher 允许使用<<_使用快照恢复集群,保存的快照>>进行恢复。如果你没有任何快照，你仍然可以<<_在没有快照的情况下恢复_etcdrke,恢复 etcd>>。
