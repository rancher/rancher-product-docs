...
* xref:manage-clusters/persistent-storage/manage-persistent-storage.adoc[持久卷声明]
...
如果你使用 ``kubectl``创建命名空间，由于 `kubectl` 不要求将新命名空间限定在你可以访问的项目内，因此你可能无法使用该命名空间。如果你的权限仅限于项目级别，则最好<<创建命名空间,通过 Rancher 创建命名空间>>，以确保你有权访问该命名空间。
====
...
使用可以分配给命名空间的项目资源（例如，xref:kubernetes-resources/workloads-and-pods/deploy-workloads.adoc[工作负载]、xref:../security/encrypting-http.adoc[证书]、xref:kubernetes-resources/configmaps.adoc[ConfigMap] 等）时，你可以动态创建命名空间。
====
...
. *可选*：如果你的项目具有有效的xref:project-admin/project-resource-quotas/project-resource-quotas.adoc[资源配额]，你可以覆盖默认资源**限制**（限制命名空间可以使用的资源）。
. 输入**名称**，然后单击**创建**。
 ...
 ** 你不能将命名空间移动到已配置xref:project-admin/project-resource-quotas/project-resource-quotas.adoc[资源配额]的项目中。
...
有关详细信息，请参阅xref:project-admin/project-resource-quotas/override-default-limit-in-namespaces.adoc[编辑命名空间资源配额]。
