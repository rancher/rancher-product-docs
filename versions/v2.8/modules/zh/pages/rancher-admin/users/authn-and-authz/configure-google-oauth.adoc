...
在 Rancher 中，只有具有 *Manage Authentication* xref:manage-role-based-access-control-rbac/global-permissions.adoc[全局角色]的管理员或用户才能配置身份验证。
...
. 使用分配了 xref:manage-role-based-access-control-rbac/global-permissions.adoc[administrator] 角色的本地用户登录到 Rancher。这个用户也称为本地主体。
. 在左上角，单击 *☰ > 用户 & 认证*。
. 在左侧导航栏，单击**认证**。
