...
* 按照以下步骤xref:migrate-to-a-new-cluster.adoc[迁移 Rancher]。
...
. 单击**创建**。
. 使用表单或 YAML 创建 Restore。如需获取使用表单创建 Restore 资源的更多信息，请参见xref:configuration/restore.adoc[配置参考]和xref:configuration/examples.adoc[示例]。
...
----
+
如需获得配置 Restore 的帮助，请参见xref:configuration/restore.adoc[配置参考]和xref:configuration/examples.adoc[示例]。
