...
| `restrictedAdmin`
| `false`
| `bool` - 如果值为 true，初始的 Rancher 用户访问本地 Kubernetes 集群会受到限制，以避免权限升级。详情请参见 link:../../rancher-admin/users/authn-and-authz/manage-role-based-access-control-rbac/global-permissions.adoc#受限管理员[restricted-admin 角色]。
|
...
有关使用私有仓库安装 Rancher 的详情，请参见xref:../other-installation-methods/air-gapped/air-gapped.adoc[离线安装]。
...
如果你使用的是私有 CA 签名的证书，请添加 `--set privateCA=true` 并参见xref:../resources/tls-secrets.adoc[添加 TLS 密文 - 使用私有 CA 签名证书]，为 Rancher 添加 CA 证书。
====
