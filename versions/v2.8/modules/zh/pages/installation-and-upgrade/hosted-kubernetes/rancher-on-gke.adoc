...
如果你已经有一个 GKE Kubernetes 集群，请直接跳转到<<_7_安装_ingress,安装 Ingress>>这个步骤。然后按照xref:../install-rancher.adoc#_安装_rancher_helm_chart[此处]的步骤安装 Rancher Helm Chart。
...
* 你需要至少一个在用的 IP 地址和至少 2 个 CPU 的云配额。有关 Rancher Server 的硬件要求，请参见xref:../requirements/requirements.adoc[本节]。
