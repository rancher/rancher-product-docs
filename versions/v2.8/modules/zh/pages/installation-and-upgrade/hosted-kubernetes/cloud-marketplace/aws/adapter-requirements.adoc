...
按照 xref:../../rancher-on-amazon-eks.adoc[Rancher 文档]创建 EKS 集群。进行到xref:../../rancher-on-amazon-eks.adoc#_8_安装_rancher_helm_chart[安装 Rancher Helm Chart]（最后一步）时，*停止并返回此页面*。该集群需要满足以下要求：
...
除了在 xref:../../rancher-on-amazon-eks.adoc#_8_安装_rancher_helm_chart[Rancher 文档]中指定的 Rancher 安装选项外，你还需要启用其它指标。
