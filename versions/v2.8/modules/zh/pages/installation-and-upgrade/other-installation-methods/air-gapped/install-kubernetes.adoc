...
本文描述了如何根据 link:../../../about-rancher/architecture/recommendations.adoc#kubernetes-安装环境[Rancher Server 环境的最佳实践]来安装 Kubernetes 集群。该集群需要专用于运行 Rancher Server。
...
. <<1-准备镜像目录,准备镜像目录>>
. <<2-创建镜像仓库-yaml,创建镜像仓库 YAML>>
. <<3-安装-k3s,安装 K3s>>
. <<4-保存并开始使用-kubeconfig-文件,保存并开始使用 kubeconfig 文件>>
...
参见link:../../troubleshooting/troubleshooting.adoc[故障排除]页面。
