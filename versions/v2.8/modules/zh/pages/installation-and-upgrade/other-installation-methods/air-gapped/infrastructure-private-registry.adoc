...
请确保你的节点满足xref:../../requirements/requirements.adoc[操作系统，容器运行时，硬件和网络]的常规要求。

如需获取配置 Linux 节点的示例，请参见xref:../../infrastructure-setup/nodes-in-amazon-ec2.adoc[在 Amazon EC2 中配置节点]的教程。
...
如需获取配置数据库示例，请参见xref:../../infrastructure-setup/mysql-database-in-amazon-rds.adoc[在 Amazon RDS 服务中配置 MySQL 数据库]的教程。
