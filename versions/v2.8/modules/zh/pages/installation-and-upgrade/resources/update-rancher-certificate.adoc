...
.重要：
====
由于证书由私有 CA 签发，因此确保在 `values.yaml` 文件中设置了 xref:../references/helm-chart-options.adoc#_常用选项[`privateCA: true`] 是非常重要的。
====
...
. 更新 `values.yaml` 文件中的当前 Helm 值：
 ...
 .. 根据需要调整 `ingress.tls.source` 字段。有关更多信息，请参阅 xref:../references/helm-chart-options.adoc#_常用选项[Chart 选项]。以下是一些示例：
