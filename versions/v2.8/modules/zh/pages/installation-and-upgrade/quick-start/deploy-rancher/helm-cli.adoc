...
这些说明还省略了完整的安装要求和其他安装选项。如果你对这些步骤有任何疑问，请参阅完整的 link:../../install-rancher.adoc[Helm CLI 安装文档]。

要使用新的 Rancher Server 来启动新的 Kubernetes 集群，你可能需要在 Rancher 中设置云凭证。有关更多信息，请参阅link:../../../cluster-deployment/launch-kubernetes-with-rancher.adoc[使用 Rancher 启动 Kubernetes 集群]。
