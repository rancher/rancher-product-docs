...
下图描述了为每个xref:../../cluster-deployment/cluster-deployment.adoc[集群类型]打开的端口。+++<figcaption>+++Rancher 管理面板的端口要求+++</figcaption>+++
...
有关 Harvester 端口要求的更多信息，请参阅link:../../integrations/harvester/overview.adoc#端口要求[此处]。
...
. 导航到**安全和用户** > *防火墙* > *区域：公共* > *端口*。要在界面内导航，请参照https://doc.opensuse.org/documentation/leap/reference/html/book-reference/cha-yast-text.html#sec-yast-cli-navigate[说明]。
