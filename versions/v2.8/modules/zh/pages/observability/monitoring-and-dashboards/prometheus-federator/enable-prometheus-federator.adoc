...
默认情况下，Prometheus Federator 已配置并旨在与 xref:../monitoring-and-alerting.adoc[rancher-monitoring] 一起部署。rancher-monitoring 同时部署了 Prometheus Operator 和 Cluster Prometheus，每个项目监控堆栈（Project Monitoring Stack）默认会联合命名空间范围的指标。
