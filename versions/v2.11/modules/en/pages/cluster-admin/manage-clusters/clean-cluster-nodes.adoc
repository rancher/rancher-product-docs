...
|===
| Removed Component | {xref-infra-providers}[Nodes Hosted by Infrastructure Provider] | {xref-custom-clusters}[Custom Nodes] | xref:cluster-deployment/hosted-kubernetes/hosted-kubernetes.adoc[Hosted Cluster] | {xref-register-clusters}[Registered Nodes]
