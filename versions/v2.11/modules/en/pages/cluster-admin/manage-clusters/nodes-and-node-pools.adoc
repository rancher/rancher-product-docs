...
|===
| Option | {xref-infra-providers}[Nodes Hosted by an Infrastructure Provider] | xref:cluster-deployment/custom-clusters/custom-clusters.adoc[Custom Node] | xref:cluster-deployment/hosted-kubernetes/hosted-kubernetes.adoc[Hosted Cluster] | {xref-register-clusters}[Registered EKS Nodes] | {xref-register-clusters}[All Other Registered Nodes] | Description
