...
:page-community-path: how-to-guides/new-user-guides/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.adoc 
:page-product-path: cluster-admin/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.adoc
...
This section intended to help you set up an alternative method to access an {xref-launch-k8s-with-rancher}[RKE cluster.]

This method is only available for RKE, RKE2, and K3s clusters that have the {xref-arch-com-ds-cluster}#_4_authorized_cluster_endpoint[authorized cluster endpoint] enabled. When Rancher creates the cluster, it generates a kubeconfig file that includes additional kubectl context(s) for accessing your cluster. This additional context allows you to use kubectl to authenticate with the downstream cluster without authenticating through Rancher. For a longer explanation of how the authorized cluster endpoint works, refer to {xref-kubeconfig-file}[this page].
