...
In order to dynamically provision storage in vSphere, the vSphere provider must be enabled. See the following pages for more: {xref-cloud-provider-vsphere-out-of-tree}[Out-of-tree vSphere] and xref:cluster-deployment/set-up-cloud-providers/configure-in-tree-vsphere.adoc[in-tree vSphere].

== Prerequisites

In order to provision vSphere volumes in a cluster created with the {xref-launch-k8s-with-rancher}[Rancher Kubernetes Engine (RKE)], the https://rancher.com/docs/rke/latest/en/config-options/cloud-providers/vsphere[vSphere cloud provider] must be explicitly enabled in the {xref-rke1-config}[cluster options].
