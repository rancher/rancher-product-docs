...
:page-community-path: how-to-guides/new-user-guides/backup-restore-and-disaster-recovery/restore-rancher-launched-kubernetes-clusters-from-backup.adoc 
:page-product-path: cluster-admin/backups-and-restore/restore.adoc
...
Rancher recommends enabling the {xref-ds-backup}#_configuring_recurring_snapshots[ability to set up recurring snapshots of etcd], but {xref-ds-backup}#_one_time_snapshots[one-time snapshots] can easily be taken as well. Rancher allows restore from <<_restoring_a_cluster_from_a_snapshot,saved snapshots>> or if you don't have any snapshots, you can still <<_recovering_etcd_without_a_snapshot_rke,restore etcd>>.
...
. Remove all etcd and control plane nodes from your cluster.
 .. In the upper left corner, click *☰ > Cluster Management*.
...
. After all etcd and control plane nodes are removed, add the new etcd / control plane node that you are planning to restore from. Assign the new node the role etcd and control plane (etcd, controlplane).
...
. After the single nodes is up and running, Rancher recommends adding additional etcd nodes to your cluster. If you have a {xref-custom-clusters}[custom cluster] and you want to reuse an old node, you are required to {xref-clean-cluster-nodes}#_cleaning_up_nodes[clean up the nodes] before attempting to add them back into a cluster.
