...
:page-community-path: troubleshooting/other-troubleshooting-tips/dns.adoc 
:page-product-path: troubleshooting/other-troubleshooting-tips/dns.adoc
...
Before running the DNS checks, check the {xref-rke1-config}#_default_dns_provider[default DNS provider] for your cluster and make sure that xref:./networking.adoc#_check_if_overlay_network_is_functioning_correctly[the overlay network is functioning correctly] as this can also be the reason why DNS resolution (partly) fails.
...
See {xref-rke1-config}#_editing_clusters_with_yaml[Editing Cluster as YAML] how to apply this change. When the provisioning of the cluster has finished, you have to remove the kube-dns pod to activate the new setting in the pod:
