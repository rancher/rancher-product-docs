...
:xref-server-side-pagination: xref:rancher-admin/experimental-features/ui-server-side-pagination.adoc
:xref-cluster-role-aggregation: xref:rancher-admin/experimental-features/cluster-role-aggregation.adoc
endif::[]
ifeval::["{build-type}" == "community"]
...
:xref-server-side-pagination: xref:how-to-guides/advanced-user-guides/enable-experimental-features/ui-server-side-pagination.adoc
:xref-cluster-role-aggregation: xref:how-to-guides/advanced-user-guides/enable-experimental-features/cluster-role-aggregation.adoc
endif::[]
...
* `aggregated-roletemplates`: Use cluster role aggregation architecture for RoleTemplates, ProjectRoleTemplateBindings, and ClusterRoleTemplateBindings. See {xref-cluster-role-aggregation}[Cluster Role Aggregation] for more information.
...
To control whether Rancher should manage the Kubernetes version of imported RKE2/K3s clusters, it is recommended to use the {xref-register-clusters}#_configuring_version_management_for_suse_rancher_prime_rke2_and_suse_rancher_prime_k3s_clusters[imported-cluster-version-management] feature that is available in Rancher v2.11.0 or newer.
====

[WARNING]
====
If the `managed-system-upgrade-controller` flag was **disabled** in Rancher v2.10.x, and any imported RKE2/K3s clusters were upgraded **outside of Rancher**, follow the steps below to prevent the unexpected installation of the system-upgrade-controller app and to ensure the {xref-register-clusters}#_configuring_version_management_for_suse_rancher_prime_rke2_and_suse_rancher_prime_k3s_clusters[imported-cluster-version-management] feature works correctly:
...
You can enable this feature on a per-cluster basis. For more information, please refer to the {xref-register-clusters}#_configuring_version_management_for_suse_rancher_prime_rke2_and_suse_rancher_prime_k3s_clusters[documentation].
====
...
* `unsupported-storage-drivers`: Enables types for storage providers and provisioners that aren't enabled by default. See {xref-unsupported-storage-drivers}[Allow Unsupported Storage Drivers] for more information.
* `ui-sql-cache`: Enables a SQLite-based cache for UI tables. See {xref-server-side-pagination}[UI Server-Side Pagination] for more information.
