...
:page-community-path: reference-guides/cluster-configuration/rancher-server-configuration/use-existing-nodes/rancher-agent-options.adoc 
:page-product-path: cluster-deployment/custom-clusters/rancher-agent-options.adoc
...
Rancher deploys an agent on each node to communicate with the node. This pages describes the options that can be passed to the agent. To use these options, you will need to {xref-custom-clusters}[create a cluster with custom nodes] and add the options to the generated `docker run` command when adding a node.

For an overview of how Rancher communicates with downstream clusters using node agents, refer to the {xref-arch-com-ds-cluster}#_3_node_agents[architecture section.]
