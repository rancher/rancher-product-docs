...
:page-community-path: reference-guides/cluster-configuration/rancher-server-configuration/rke1-cluster-configuration.adoc 
:page-product-path: cluster-deployment/configuration/rke1.adoc
...
:xref-global-default-private-registry: xref:rancher-admin/global-configuration/global-default-private-registry.adoc
endif::[]
ifeval::["{build-type}" == "community"]
...
:xref-global-default-private-registry: xref:how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/global-default-private-registry.adoc
endif::[]

When Rancher installs Kubernetes, it uses {xref-launch-k8s-with-rancher}[RKE] or {url-rke2-introduction}[RKE2] as the Kubernetes distribution.
...
In {xref-launch-k8s-with-rancher}[clusters launched by RKE], you can edit any of the remaining options that follow.
...
There are two main ways to set up private registries in Rancher: by setting up the {xref-global-default-private-registry}[global default registry] through the *Settings* tab in the global view, and by setting up a private registry in the advanced options in the cluster-level settings. The global default registry is intended to be used for air-gapped setups, for registries that do not require credentials. The cluster-level private registry is intended to be used in all setups in which the private registry requires credentials.
...
ACE must be set up {xref-register-clusters}#_authorized_cluster_endpoint_support_for_rke2_and_k3s_clusters[manually] on RKE2 and K3s clusters. In RKE, ACE is enabled by default in Rancher-launched Kubernetes clusters, using the IP of the node with the `controlplane` role and the default Kubernetes self-signed certificates.
...
For information on using the Rancher UI to set up node pools in an RKE cluster, refer to {xref-infra-providers}[this page.]
...
Option to set environment variables for {xref-about-rancher-agents}[rancher agents]. The environment variables can be set using key value pairs. If rancher agent requires use of proxy to communicate with Rancher server, `HTTP_PROXY`, `HTTPS_PROXY` and `NO_PROXY` environment variables can be set using agent environment variables.
...
Option to enable or disable {xref-monitoring-and-dashboards}[Cluster Monitoring].
