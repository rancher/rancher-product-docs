...
:page-community-path: how-to-guides/new-user-guides/launch-kubernetes-with-rancher/index.adoc 
:page-product-path: cluster-deployment/launch-kubernetes-with-rancher.adoc
...
If you use RKE to set up a cluster, your nodes must meet the {xref-node-reqs}[requirements] for nodes in downstream user clusters.

=== Launching Kubernetes on New Nodes in an Infrastructure Provider

Using Rancher, you can create pools of nodes based on a {xref-infra-providers}#_node_templates[node template]. This node template defines the parameters you want to use to launch nodes in your cloud providers.

One benefit of installing Kubernetes on node pools hosted by an infrastructure provider is that if a node loses connectivity with the cluster, Rancher can automatically create another node to join the cluster to ensure that the count of the node pool is as expected.

For more information, refer to the section on {xref-infra-providers}[launching Kubernetes on new nodes.]
...
For more information, refer to the section on {xref-custom-clusters}[custom nodes.]
