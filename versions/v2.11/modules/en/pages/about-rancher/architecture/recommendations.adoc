...
For more best practices for downstream clusters, refer to the {xref-production-checklist}[production checklist] or our {xref-best-practices}[best practices guide.]

== Architecture for an Authorized Cluster Endpoint (ACE)

If you are using an {xref-arch-com-ds-cluster}#_4_authorized_cluster_endpoint[authorized cluster endpoint (ACE),] we recommend creating an FQDN pointing to a load balancer which balances traffic across your nodes with the `controlplane` role.
