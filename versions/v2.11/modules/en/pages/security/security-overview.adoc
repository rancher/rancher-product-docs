...
:xref-k8s-security-best-practices: xref:security/kubernetes-security-best-practices.adoc
endif::[]
ifeval::["{build-type}" == "community"]
:url-security-overview: https://open-docs.neuvector.com/basics/overview
...
:xref-k8s-security-best-practices: xref:reference-guides/rancher-security/kubernetes-security-best-practices.adoc
endif::[]
...
For details, refer to the section on {xref-cis-howto}[security scans].
...
We provide two RPMs (Red Hat packages) that enable Rancher products to function properly on SELinux-enforcing hosts: `rancher-selinux` and `rke2-selinux`. For details, see {xref-selinux-rpm}[this page].
...
For recommendations on securing your Kubernetes cluster, refer to the {xref-k8s-security-best-practices}[Kubernetes Security Best Practices] guide.
