...
:page-community-path: reference-guides/rancher-security/hardening-guides/rke1-hardening-guide/index.adoc 
:page-product-path: security/hardening-guides/rke1/rke1.adoc
...
The example `cluster.yml` configuration file contains an Admission Configuration policy in the `services.kube-api.admission_configuration` field. This {xref-sample-psact}[sample] policy contains the namespace exemptions necessary for an imported RKE cluster to run properly in Rancher, similar to Rancher's pre-defined {xref-psact}[`rancher-restricted`] policy.
...
[NOTE]
====
If you intend to import an RKE cluster into Rancher, please consult the {xref-psact}[documentation] for how to configure the PSA to exempt Rancher system namespaces.
====
