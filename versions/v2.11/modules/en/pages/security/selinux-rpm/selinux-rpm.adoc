= SELinux RPM
:page-languages: [en, zh]
:revdate: 2026-07-14
:page-revdate: {revdate}
:page-community-path: reference-guides/rancher-security/selinux-rpm/index.adoc 
:page-product-path: security/selinux-rpm/selinux-rpm.adoc
ifeval::["{build-type}" != "community"]
:xref-rancher-selinux: xref:security/selinux-rpm/about-rancher-selinux.adoc
:xref-rke2-selinux: xref:security/selinux-rpm/about-rke2-selinux.adoc
endif::[]
ifeval::["{build-type}" == "community"]
:xref-rancher-selinux: xref:reference-guides/rancher-security/selinux-rpm/about-rancher-selinux.adoc
:xref-rke2-selinux: xref:reference-guides/rancher-security/selinux-rpm/about-rke2-selinux.adoc
endif::[]

https://en.wikipedia.org/wiki/Security-Enhanced_Linux[Security-Enhanced Linux (SELinux)] is a security enhancement to Linux.

Developed by Red Hat, it is an implementation of mandatory access controls (MAC) on Linux. Mandatory access controls allow an administrator of a system to define how applications and users can access different resources such as files, devices, networks and inter-process communication. SELinux also enhances security by making an OS restrictive by default.

After being historically used by government agencies, SELinux is now industry standard and is enabled by default on CentOS 7 and 8. To check whether SELinux is enabled and enforcing on your system, use `getenforce`:

[,shell]
----
# getenforce
Enforcing
----

We provide two RPMs (Red Hat packages) that enable Rancher products to function properly on SELinux-enforcing hosts: {xref-rancher-selinux}[`rancher-selinux`] and {xref-rke2-selinux}[`rke2-selinux`].
