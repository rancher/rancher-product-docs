...
. 打开浏览器并导航到 https://github.com/rancher/rancher/blob/main/cleanup/user-cluster.sh[GitHub]，然后下载 `user-cluster.sh`。
...
. 从节点中<<_docker_containers_images_and_volumes,删除>> Docker 容器并<<_挂载,卸载>>任何卷。
. 重启节点。
. <<_目录和文件,删除>>所有剩余的文件。
...
[pass]
<h3><a id="_docker_containers_images_and_volumes"></a>Docker 容器、镜像和卷</h3>
