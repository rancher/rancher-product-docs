...
In order to provision vSphere volumes in a cluster created with the {xref-launch-k8s-with-rancher}[Rancher Kubernetes Engine (RKE)], the https://rancher.com/docs/rke/latest/en/config-options/cloud-providers/vsphere[vSphere cloud provider] must be explicitly enabled in the xref:cluster-deployment/configuration/rke1.adoc[cluster options].
