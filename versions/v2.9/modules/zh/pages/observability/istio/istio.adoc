...
你仍然可以通过安装 xref:observability/monitoring-and-dashboards/monitoring-and-alerting.adoc[Rancher Monitoring] 或安装你自己的 Prometheus operator 来使用 Prometheus 集成。Rancher 的 Istio chart 还默认安装 Kiali，确保你可以开箱即用地全面了解微服务。
...
如果要在 RKE2 集群上安装 Istio，则需要执行一些额外的步骤。有关详细信息，请参阅<<_在_rke2_集群上安装_istio_的其他步骤,本节>>
