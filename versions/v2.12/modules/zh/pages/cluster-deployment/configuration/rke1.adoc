...
Rancher 安装 Kubernetes 时，它使用 xref:cluster-deployment/launch-kubernetes-with-rancher.adoc[RKE] 或 https://documentation.suse.com/cloudnative/rke2/latest/zh/introduction.html[RKE2] 作为 Kubernetes 发行版。
...
* <<_rke_集群配置文件参考,集群配置文件>>：高级用户可以创建一个 RKE 配置文件，而不是使用 Rancher UI 来为集群选择 Kubernetes 选项。配置文件可以让你使用 YAML 来指定 RKE 安装中可用的任何选项（除了 system_images 配置）。

RKE 集群配置选项嵌套在 `rancher_kubernetes_engine_config` 参数下。有关详细信息，请参阅<<_rke_集群配置文件参考,集群配置文件>>。

在 xref:cluster-deployment/launch-kubernetes-with-rancher.adoc[RKE 启动的集群]中，你可以编辑任何后续剩余的选项。
...
这指的是集群使用的link:https://kubernetes.io/docs/concepts/cluster-administration/networking/[网络提供商]。有关不同网络提供商的更多详细信息，请查看我们的xref:faq/container-network-interface-providers.adoc[网络常见问题解答]。
...
include::shared:ROOT:partial$zh/deprecation-weave.adoc[]

[NOTE]
.Weave 注意事项：
====
选择 Weave 作为网络提供商时，Rancher 将通过生成随机密码来自动启用加密。如果你想手动指定密码，请参阅使用<<_rke_集群配置文件参考,配置文件>>和 https://rancher.com/docs/rke/latest/en/config-options/add-ons/network-plugins/#weave-network-plug-in-options[Weave 网络插件选项]来配置集群。
====
...
如果你要使用的云提供商未作为选项列出，你需要使用<<_rke_集群配置文件参考,配置文件选项>>来配置云提供商。请参考 https://rancher.com/docs/rke/latest/en/config-options/cloud-providers/[RKE 云提供商文档]来了解如何配置云提供商。
====
...
在 Rancher 中设置私有镜像仓库的主要方法有两种：通过xref:rancher-admin/global-configuration/global-default-private-registry.adoc[全局默认镜像仓库]中的**设置**选项卡设置全局默认镜像仓库，以及在集群级别设置的高级选项中设置私有镜像仓库。全局默认镜像仓库可以用于离线设置，不需要凭证的镜像仓库。而集群级私有镜像仓库用于所有需要凭证的私有镜像仓库。

如果你的私有镜像仓库需要凭证，为了将凭证传递给 Rancher，你需要编辑每个需要从仓库中拉取镜像的集群的集群选项。

私有镜像仓库的配置选项能让 Rancher 知道要从哪里拉取用于集群的link:https://rancher.com/docs/rke/latest/en/config-options/system-images/[系统镜像]或link:https://rancher.com/docs/rke/latest/en/config-options/add-ons/[附加组件镜像]。

* **系统镜像**是维护 Kubernetes 集群所需的组件。
* **附加组件**用于部署多个集群组件，包括网络插件、ingress controller、DNS 提供商或 metrics server。

有关为集群配置期间应用的组件设置私有镜像仓库的更多信息，请参阅link:https://rancher.com/docs/rke/latest/en/config-options/private-registries/[私有镜像仓库的 RKE 文档]。

Rancher v2.6 引入了link:https://rancher.com/docs/rke/latest/en/config-options/private-registries/#amazon-elastic-container-registry-ecr-private-registry-setup[为 RKE 集群配置 ECR 镜像仓库]的功能。
...
授权集群端点仅适用于 Rancher 启动的 Kubernetes 集群，即只适用于 Rancher xref:about-rancher/architecture/communicating-with-downstream-clusters.adoc#_配置_kubernetes_集群的工具[使用 RKE] 来配置的集群。它不适用于托管在 Kubernetes 提供商中的集群，例如 Amazon 的 EKS。
====
...
有关使用 Rancher UI 在 RKE 集群中设置节点池的信息，请参阅xref:cluster-deployment/infra-providers/infra-providers.adoc[此页面]。
...
为集群启用 xref:security/psp/create.adoc[pod 安全策略]。启用此选项后，使用**默认 Pod 安全策略**下拉菜单选择一个策略。
...
如需了解各个 Rancher 版本通过了哪些 Docker 版本测试，请参见link:https://rancher.com/support-maintenance-terms/[支持和维护条款]。
...
启用或禁用link:https://rancher.com/docs/rke/latest/en/etcd-snapshots/#etcd-recurring-snapshots[定期 etcd 快照]的选项。

=== Agent 环境变量

为 xref:cluster-deployment/about-rancher-agents.adoc[rancher agent] 设置环境变量的选项。你可以使用键值对设置环境变量。如果 Rancher Agent 需要使用代理与 Rancher Server 通信，则可以使用 Agent 环境变量设置 `HTTP_PROXY`，`HTTPS_PROXY` 和 `NO_PROXY` 环境变量。
...
=== Cluster Agent 配置和 {fleet-product-name} Agent 配置
...
高级用户可以创建一个 RKE 配置文件，而不是使用 Rancher UI 来为集群选择 Kubernetes 选项。配置文件可以让你在 RKE 安装中设置任何link:https://rancher.com/docs/rke/latest/en/config-options/[可用选项]（`system_images` 配置除外）。使用 Rancher UI 或 API 创建集群时，不支持 `system_images` 选项。
...
请参阅 <<_docker_根目录,Docker 根目录>>。

=== enable_cluster_monitoring

启用或禁用xref:observability/monitoring-and-dashboards/monitoring-and-dashboards.adoc[集群监控]的选项。
...
你可以使用 RKE 的link:https://rancher.com/docs/rke/latest/en/config-options/add-ons/user-defined-add-ons/[用户定义的附加组件功能]来添加自定义网络插件。部署 Kubernetes 集群之后，你可以定义要部署的任何附加组件。
