...
ifeval::["{build-type}" != "community"]
xef:observability/istio/istio.adoc[Rancher-Istio] has been deprecated in Rancher v2.12.0; turn to the https://apps.rancher.io[SUSE Application Collection] build of Istio for enhanced security (included in SUSE Rancher Prime subscriptions). Detailed information can be found in https://forums.suse.com/t/deprecation-of-rancher-istio/45043[this announcement].
endif::[]
====
...
See the full list of https://github.com/rancher/rancher/compare/v2.12.5...v2.12.6[changes].
...
ifeval::["{build-type}" != "community"]
If you're installing Rancher for the first time, your environment must fulfill the xef:installation-and-upgrade/requirements/requirements.adoc[installation requirements].
endif::[]
...
ifeval::["{build-type}" != "community"]
Rancher now has an enablement option called xef:observability/logging/enable-api-audit-log.adoc#_api_audit_log_options[`AUDIT_LOG_ENABLED`] for API Audit Logs for a Rancher installation. In Rancher versions 2.11.x and earlier, only the `AUDIT_LEVEL` could be set and the default log level (`0`) would disable the audit log. In Rancher versions 2.12.x and later, the default log level (`0`) now only contains the log request and response metadata, and can be set when configuring `AUDIT_LOG_ENABLED`. If installing or upgrading via Helm you can enable the API Audit Logs and specify the log level by applying the following setting to your Helm command: `--set auditLog.enabled=true --set auditLog.level=0`. See the xef:observability/logging/enable-api-audit-log.adoc[Enabling the API Audit Log to Record System Events] documentation and https://github.com/rancher/rancher/issues/48941[#48941].
endif::[]
====
...
ifeval::["{build-type}" != "community"]
** When using a proxy in front of an air-gapped Rancher instance, you must pass additional parameters to `NO_PROXY`. See the xef:installation-and-upgrade/other-installation-methods/http-proxy/install-rancher.adoc[documentation] and issue https://github.com/rancher/docs/issues/2725#issuecomment-702454584[#2725].
...
* rancher/rancher:v2.12.6

== Tools

* CLI - https://github.com/rancher/cli/releases/tag/v2.12.6[v2.12.6]
