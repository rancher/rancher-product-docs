...
:page-community-path: how-to-guides/new-user-guides/kubernetes-resources-setup/configmaps.adoc 
:page-product-path: cluster-admin/kubernetes-resources/configmaps.adoc
:experimental:
...
. On the *Data* tab, add a key-value pair to your ConfigMap. Add as many values as you need.  You can add multiple key value pairs to the ConfigMap by copying and pasting. Alternatively, use *Read from File* to add the data. Note: If you need to store sensitive data, {xref-secrets}[use a secret], not a ConfigMap.
. Click *Create*.
...
For more information on adding ConfigMaps to a workload, see {xref-deploy-workloads}[Deploying Workloads].
