...
ifeval::["{build-type}" != "community"]
xref:observability/istio/istio.adoc[Rancher-Istio] has been deprecated in Rancher v2.12.0; turn to the https://apps.rancher.io[SUSE Application Collection] build of Istio for enhanced security (included in SUSE Rancher Prime subscriptions). Detailed information can be found in https://forums.suse.com/t/deprecation-of-rancher-istio/45043[this announcement].
endif::[]
====

ifeval::["{build-type}" != "community"]
Rancher v2.13.2 is the latest patch release of Rancher. This is a Prime version release that introduces new features, enhancements, and bug fixes. To learn more about Rancher Prime, see our page on the https://www.rancher.com/products/rancher-platform[Rancher Prime Platform].

== What's New - SUSE Rancher Prime v2.13.2
...
* Starting with Rancher Prime v2.13.1, the chart name has changed from `rancher` to `rancher-prime`. This means when you upgrade or install Rancher v2.13.1 or higher, the chart name you use will be `rancher-prime` instead. As an example the installation command is now `helm install rancher rancher-prime/rancher-prime`. Note that you must first set up the `rancher-prime` chart repository.
endif::[]
...
* Fixed an issue where using Cluster Autoscaler with K3s clusters during node removal while scaling down a `machinePool`, it was possible that a node was immediately re-added before the pool was resized. This was a race condition and eventually the pool was reduced to the required size based on the workload. See https://github.com/rancher/rancher/issues/52859[#52859].
endif::[]
...
====
Starting with Rancher Prime v2.13.1, the chart name has changed from `rancher` to `rancher-prime`. This means when you upgrade or install Rancher v2.13.1 or higher, the chart name you use will be `rancher-prime` instead, for example `helm install rancher rancher-prime/rancher-prime`. Note that you must first set up the `rancher-prime` chart repository.
...
ifeval::["{build-type}" != "community"]
Refer to the xref:api/extension-apiserver.adoc#_aggregation_layer_is_required[Extension API Server documentation] and https://github.com/rancher/rancher/issues/50400[#50400] for more information.
endif::[]
====
...
ifeval::["{build-type}" != "community"]
For additional information refer to the xref:rancher-admin/global-configuration/ui-server-side-pagination.adoc#_disk_space[UI Server Side Pagination - Disk Space documentation].
endif::[]
====
...
ifeval::["{build-type}" != "community"]
* *Creating backups:* xref:rancher-admin/back-up-restore-and-disaster-recovery/back-up.adoc[Create a backup] before you upgrade Rancher. To roll back Rancher after an upgrade, you must first back up and restore Rancher to the previous Rancher version. Because Rancher will be restored to the same state as when the backup was created, any changes post-upgrade will not be included after the restore.
endif::[]
...
* CLI - https://github.com/rancher/cli/releases/tag/v2.13.2[v2.13.2]
