...
* *Updating template settings:* If an organization's security and DevOps teams decide to embed best practices into the required settings for new clusters, those best practices could change over time. If the best practices change, xref:./example-use-cases.adoc#_updating_templates_and_clusters_created_with_them[a template can be updated to a new revision] and clusters created from the template can {xref-manage-rke1-templates}#_upgrading_a_cluster_to_use_a_new_template_revision[upgrade to the new version] of the template.
...
You can xref:./apply-templates.adoc#_creating_a_cluster_from_an_rke_template[create a cluster from a template] that you created, or from a template that has been {xref-access-share-rke1-templates}[shared with you.]
