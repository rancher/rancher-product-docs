...
==== Worker Compliance 配置文件

选择一个 xref:security/compliance-scans/how-to.adoc[compliance benchmark] 来验证系统配置。
...
高级用户可以创建一个配置文件，而不是使用 Rancher UI 来为集群选择 Kubernetes 选项。配置文件允许你为 RKE2 设置link:https://documentation.suse.com/cloudnative/rke2/latest/zh/install/configuration.html[可用的选项]，其中包括已经在 <<_rancher_ui_中的配置选项,Rancher UI 配置选项>>中列出的选项以及 Rancher 特定的参数。
