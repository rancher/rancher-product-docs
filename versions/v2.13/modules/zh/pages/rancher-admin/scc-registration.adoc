...
After you have your registration code, you can register your Rancher Manager instance through Helm by passing the argument `--set registration.enabled=true --set registration.regCode=<REGISTRATION-CODE-FROM_SCC>` in the Helm install command `helm install rancher rancher-prime/rancher-prime`.
...
. Run the Helm install command `helm install rancher rancher-prime/rancher-prime` and verify installation is successful. 
