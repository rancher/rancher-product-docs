...
:page-community-path: reference-guides/cluster-configuration/rancher-server-configuration/rke2-cluster-configuration.adoc 
:page-product-path: cluster-deployment/configuration/rke2.adoc
ifeval::["{build-type}" != "community"]
:url-rke2-configuration: https://documentation.suse.com/cloudnative/rke2/latest/en/install/configuration.html
:url-rke2-basic-network-options: https://documentation.suse.com/cloudnative/rke2/latest/en/networking/basic_network_options.html
:url-rke2-coredns: https://documentation.suse.com/cloudnative/rke2/latest/en/networking/networking_services.html#_coredns
:url-rke2-ingress-controller: https://documentation.suse.com/cloudnative/rke2/latest/en/networking/networking_services.html#_ingress_controller 
:url-rke2-automatically-deploying-manifests-and-helm-charts: https://documentation.suse.com/cloudnative/rke2/latest/en/add-ons/helm.html#_automatically_deploying_manifests_and_helm_charts
:url-rke2-linux-agent-config: https://documentation.suse.com/cloudnative/rke2/latest/en/reference/linux_agent_config.html
:url-rke2-creating-snapshots: https://documentation.suse.com/cloudnative/rke2/latest/en/backup_restore.html#_creating_snapshots
:url-rke2-dual-stack-configuration: https://documentation.suse.com/cloudnative/rke2/latest/en/networking/basic_network_options.html#_dual_stack_configuration
:url-rke2-ipv6-setup: https://documentation.suse.com/cloudnative/rke2/latest/en/networking/basic_network_options.html#_ipv6_setup
:url-rke2-containerd-registry-configuration: https://documentation.suse.com/cloudnative/rke2/latest/en/install/containerd_registry_configuration.html
:url-rke2-helm: https://documentation.suse.com/cloudnative/rke2/latest/en/add-ons/helm.html
:url-rke2-server-config: https://documentation.suse.com/cloudnative/rke2/latest/en/reference/server_config.html
...
ifeval::["{build-type}" == "community"]
:url-rke2-configuration: https://docs.rke2.io/install/configuration
:url-rke2-basic-network-options: https://docs.rke2.io/networking/basic_network_options
:url-rke2-coredns: https://docs.rke2.io/networking/networking_services#coredns
:url-rke2-ingress-controller: https://docs.rke2.io/networking/networking_services#ingress-controller
:url-rke2-automatically-deploying-manifests-and-helm-charts: https://docs.rke2.io/add-ons/helm#automatically-deploying-manifests-and-helm-charts
:url-rke2-linux-agent-config: https://docs.rke2.io/reference/linux_agent_config
:url-rke2-creating-snapshots: https://docs.rke2.io/datastore/backup_restore#creating-snapshots
:url-rke2-dual-stack-configuration: https://docs.rke2.io/networking/basic_network_options#dual-stack-configuration
:url-rke2-ipv6-setup: https://docs.rke2.io/networking/basic_network_options#ipv6-setup
:url-rke2-containerd-registry-configuration: https://docs.rke2.io/install/private_registry
:url-rke2-helm: https://docs.rke2.io/add-ons/helm
:url-rke2-server-config: https://docs.rke2.io/reference/server_config
...
* <<_cluster_config_file_reference,Cluster Config File>>: Instead of using the Rancher UI to choose Kubernetes options for the cluster, advanced users can create an RKE2 config file. Using a config file allows you to set many additional {url-rke2-configuration}[options] available for an RKE2 installation.
...
For a complete reference of configurable options for RKE2 clusters in YAML, see the {url-rke2-configuration}[RKE2 documentation].
...
For more details on the different networking providers and how to configure them, please view our {url-rke2-basic-network-options}[RKE2 documentation].
...
By default, https://coredns.io/[CoreDNS] is installed as the default DNS provider. If CoreDNS is not installed, an alternate DNS provider must be installed yourself. Refer to the {url-rke2-coredns}[RKE2 documentation] for additional CoreDNS configurations.
...
This is the legacy ingress option to enable Ingress-NGINX within the cluster. Refer to the {url-rke2-ingress-controller}[RKE2 documentation] for additional configuration options.
--

Traefik::
+
--
If you want to expose your applications in a high-availability configuration, and you're hosting your nodes with a cloud provider that doesn't have a native load-balancing feature, enable this option to use Traefik Ingress within the cluster. Refer to the {url-rke2-ingress-controller}[RKE2 documentation] for additional configuration options.
--
======
...
Additional Kubernetes manifests, managed as an https://kubernetes.io/docs/concepts/cluster-administration/addons/[Add-on], to apply to the cluster on startup. Refer to the {url-rke2-automatically-deploying-manifests-and-helm-charts}[RKE2 documentation] for details.

[#_agent_environment_vars]
==== Agent Environment Vars

Option to set environment variables for {xref-about-rancher-agents}[Rancher agents]. The environment variables can be set using key value pairs. Refer to the {url-rke2-linux-agent-config}[RKE2 documentation] for more details.
...
Option to enable or disable recurring etcd snapshots. If enabled, users have the option to configure the frequency of snapshots. For details, refer to the {url-rke2-creating-snapshots}[RKE2 documentation]. Note that with RKE2, snapshots are stored on each etcd node. This varies from RKE1 which only stores one snapshot per cluster.
...
* {url-rke2-dual-stack-configuration}[RKE2 documentation: Dual-stack configuration]
* {url-rke2-ipv6-setup}[RKE2 documentation: IPv6-only setup]
...
Select the image repository to pull Rancher images from. For more details and configuration options, see the {url-rke2-containerd-registry-configuration}[RKE2 documentation].
...
Editing clusters in YAML allows you to set the {url-rke2-configuration}[options available] in an RKE2 installation, including those already listed in <<_configuration_options_in_the_rancher_ui,Configuration Options in the Rancher UI>>, as well as set Rancher-specific parameters.
...
For more information about how RKE2 manges packaged components, please refer to {url-rke2-helm}[RKE2 documentation].
...
Specify RKE2 configurations. Any configuration change made here will apply to every node. The configuration options available in the {url-rke2-server-config}[standalone version of RKE2] can be applied here.
