...
For maximum uptime and stability, the {rancher-product-name} server cluster requires three nodes, each assigned all roles (etcd, control plane, and worker). For further optimization in large environments, see {xref-tuning-rancher-at-scale}[Tuning and Best Practices for SUSE Rancher Prime at Scale].
