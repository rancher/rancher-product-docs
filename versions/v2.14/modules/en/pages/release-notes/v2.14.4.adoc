...
:release-version: v2.14.4
:rn-component-version: v2.14
:previous-release-version: v2.14.3
...
* Fixed a performance issue that could cause Deployment, StatefulSet, and DaemonSet detail pages to become unresponsive in namespaces with large numbers of workloads, services, pods, and ingresses. See https://github.com/rancher/dashboard/issues/18377[#18377].
* Fixed the *Show Configuration* dialog for Project Monitors so the *Config* tab is read-only and no longer shows unsaved values when reopened. See https://github.com/rancher/dashboard/issues/18166[#18166].

[#_known_issues_rancher_app_global_ui]
=== Known Issues

* Community releases can display incorrect end-of-life notification text. The notification may refer to end-of-life and support status instead of stating that the version will no longer receive updates. See https://github.com/rancher/dashboard/issues/18321[#18321].
* In *Cluster Management* > *Local Cluster* > *Node Pools*, node internal and external IP addresses may not appear in the UI. See https://github.com/rancher/dashboard/issues/18097[#18097].
...
* On RKE2 clusters that use Traefik v40 or later, enabling *Dual-Mode* for the ingress controller from the UI can write the migration configuration under the wrong key, so the migration ingressClass is not created. This affects RKE2 versions v1.36.2+rke2r1+, v1.35.6+rke2r1+, v1.34.9+rke2r1+, and v1.33.13+rke2r1+. See https://github.com/rancher/dashboard/issues/18260[#18260].
...
* Fixed an issue where the Rancher Backup Operator could crash during startup when metrics collection was enabled. See https://github.com/rancher/backup-restore-operator/issues/1023[#1023].
