...
SUSE Rancher v2.14 adds support for Cluster API **v1beta2** by upgrading to **cluster-api v1.12.2** (from **v1.10.6**). Rolling back to **Rancher 2.13.x**, which supports Cluster API **v1beta1**, introduces compatibility issues and requires alternative rollback steps. Refer to the https://documentation.suse.com/cloudnative/rancher-manager/v2.13/en/installation-and-upgrade/rollbacks.html#_alternative_steps_for_special_scenarios[Rancher documentation] for the required procedure. Refer to {rn-capi-v1beta2}[Upgrading to CAPI v1beta2] for more details on the changes introduced in this new API version.
