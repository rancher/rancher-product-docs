...
ifeval::["{build-type}" != "community"]
:rn-capi-infra-providers: https://documentation.suse.com/cloudnative/rancher-manager/v2.14/en/cluster-deployment/configuration/capi-infrastructure-providers.html
:rn-capi-overview: https://documentation.suse.com/cloudnative/rancher-manager/v2.14/en/integrations/cluster-api/overview.html
:rn-rollbback: https://documentation.suse.com/cloudnative/rancher-manager/v2.14/en/installation-and-upgrade/rollbacks.html#_alternative_steps_for_special_scenarios
...
//Community release content section
//ifeval::["{build-type}" == "community"]
//
//endif::[]
...
//Community v2.14.1 known issue, remove conditional in following release
ifeval::["{build-type}" == "community"]
...
ifeval::["{build-type}" == "community"]
* *Rancher v2.14.0:*
** After updating an RKE2 cluster to a supported Kubernetes version, the S3 snapshot retention defaults to `5`, ignoring any previously set `snapshotRetention` values. This is caused by a known issue in RKE2, tracked in https://github.com/k3s-io/k3s/issues/13769[#13769], which is targeted for a fix in a future KDM update. As a workaround, Rancher v2.14.0 introduces the ability to explicitly specify `etcd-s3-retention` to manage S3 snapshot retention independently. See also https://github.com/rancher/rancher/issues/53046[#53046].
endif::[]
* *Rancher v2.7.7:*
...
ifeval::["{build-type}" == "community"]
* *Rancher v2.14.0:*
** After updating a K3s cluster to a supported Kubernetes version, the S3 snapshot retention defaults to `5`, ignoring any previously set `snapshotRetention` values. This is caused by a known issue in K3s, tracked in https://github.com/k3s-io/k3s/issues/13769[#13769], which is targeted for a fix in a future KDM update. As a workaround, Rancher v2.14.0 introduces the ability to explicitly specify `etcd-s3-retention` to manage S3 snapshot retention independently. See also https://github.com/rancher/rancher/issues/53046[#53046].
endif::[]
* *Rancher v2.7.6:*
** Provisioning RKE2/K3s clusters with added (not built-in) custom node drivers causes provisioning to fail. As a workaround, https://github.com/rancher/rancher/issues/37074#issuecomment-1664722305[fix] the added node drivers after activating. See https://github.com/rancher/rancher/issues/37074[#37074].
* *Rancher v2.7.2:*
