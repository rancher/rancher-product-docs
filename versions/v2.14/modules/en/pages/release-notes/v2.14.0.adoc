...
== What's New in Rancher v2.14.0

Rancher v2.14.0 is a significant minor release that introduces major architectural changes, new security enhancements, expanded Kubernetes version support, and improved cluster provisioning capabilities. The highlights below summarize the most impactful changes in this release.

=== Kubernetes v1.35 Support

Rancher v2.14.0 adds support for Kubernetes v1.35, the latest upstream release. At the same time, support for Kubernetes v1.32 has been removed. See https://github.com/rancher/rancher/issues/52957[#52957].

=== Gateway API Support

Rancher v2.14.0 introduces the ability to switch from Ingress to the https://gateway-api.sigs.k8s.io/[Kubernetes Gateway API] when installing the Rancher Helm chart. When using the Gateway API network exposure type, Rancher can create and manage its own Gateway resource, or work with an externally managed (shared) Gateway by providing custom HTTPRoute resources. See https://github.com/rancher/rancher/issues/52796[#52796].

=== Enhanced Authentication: PKCE and OAuth Bearer Token Support

Two key improvements harden the OIDC authentication experience:

* **PKCE (RFC 7636):** Rancher now supports the Proof Key for Code Exchange extension for OIDC providers. Enabling `S256` PKCE token verification mitigates authorization code interception attacks. See https://github.com/rancher/rancher/issues/50665[#50665].
* **OAuth/OIDC Bearer Tokens (RFC 6750):** Access tokens issued for OAuth and OIDC providers can now be used directly as standard Bearer tokens to authenticate with Rancher, in addition to the previously supported ID token. See https://github.com/rancher/rancher/issues/52716[#52716].

=== Cluster API v1beta2 Support

Rancher v2.14.0 upgrades to **cluster-api v1.12.2**, adding full support for the Cluster API **v1beta2** API. Rolling back to Rancher v2.13.x (which uses Cluster API v1beta1) requires special steps; refer to the https://ranchermanager.docs.rancher.com/v2.14/getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/rollbacks#alternative-steps-for-special-scenarios[rollback documentation]. See https://github.com/rancher/rancher/issues/52034[#52034].

=== Embedded Cluster API Removed — Rancher Turtles is Now the Default

The built-in `rancher-provisioning-capi` Cluster API functionality has been removed. Rancher Turtles is now the sole supported mechanism for Cluster API integration with Rancher. Migration from the embedded provider to Turtles happens automatically during the upgrade to v2.14.0. If you previously disabled Rancher Turtles, you must re-enable it manually. See https://github.com/rancher/rancher/issues/53291[#53291].

=== Technology Preview: Native CAPI Infrastructure Providers for RKE2

Rancher v2.14.0 introduces a technology preview allowing RKE2 clusters to be provisioned using native CAPI infrastructure providers such as https://cluster-api-aws.sigs.k8s.io/[CAPA] (Cluster API Provider AWS) and https://github.com/kubernetes-sigs/cluster-api-provider-vsphere[CAPV] (Cluster API Provider vSphere), in addition to the existing Rancher Node Driver method. This feature is experimental and should not be used for production clusters. See https://github.com/rancher/rancher/issues/51561[#51561].

=== Dual-Stack (IPv4/IPv6) Networking for EKS Clusters

Rancher now supports provisioning and managing dual-stack (IPv4/IPv6) networking for Amazon EKS clusters. See https://github.com/rancher/rancher/issues/53584[#53584].

=== Custom Labels and Annotations for Rancher Namespaces

Users can now provide custom labels and annotations to the Rancher Helm chart at install time. These are applied to Rancher-managed namespaces as they are created, in both management and downstream clusters, making it easier to identify and organize namespaces required for Rancher operations. See https://github.com/rancher/rancher/issues/53268[#53268].

=== Audit Log Policy Management in the UI

Rancher v2.14.0 adds UI support for managing audit policies under *Policy* > *Audit Log Policies*, making it easier to configure and review audit logging without direct API access. See https://github.com/rancher/dashboard/issues/12961[#12961].

=== Fleet v0.15.0 with Helm v4

Fleet has been migrated from Helm v3 to Helm v4 in version v0.15.0. Note that drift correction may not work as expected under certain circumstances with Helm v4; refer to the https://fleet.rancher.io/0.15/changelogs/v0.15.0#_drift_correction_may_not_work_as_expected_with_helm_v4[v0.15.0 changelog] for details. Additionally, the `imagescan` feature is now disabled by default and is planned for deprecation in a future release. See https://github.com/rancher/fleet/issues/4351[#4351].
