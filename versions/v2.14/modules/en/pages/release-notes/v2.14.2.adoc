...
* Starting in Rancher v2.14.1, Cluster API Addon Provider Fleet (CAAPF) is disabled by default. If you are upgrading CAPI clusters provisioned with CAAPF, keep `features.use-caapf.enabled=true` before upgrade to avoid Fleet-management handoff conflicts that can disrupt downstream workloads. See https://github.com/rancher/rancher/issues/54740[#54740].
