...
[WARNING]
====
Lorsqu'un Membre de cluster crée un projet, l'utilisateur se voit automatiquement attribuer <<#_project_roles,les privilèges de Propriétaire de projet>>. Cela leur accorde un contrôle complet sur le projet et ses ressources associées, y compris les autorisations pour déployer des charges de travail. Sans {xref-psa-pss}[Normes de sécurité des pods (PSS) et Admission de sécurité des pods (PSA)] appliquées, un Membre de cluster peut exécuter des conteneurs privilégiés dans le cluster.
====
