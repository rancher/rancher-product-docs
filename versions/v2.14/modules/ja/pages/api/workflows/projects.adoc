...
プロジェクトを作成した後、フィールド`status.backingNamespace`が入力されます。これは、プロジェクト関連リソースを管理するために作成された管理クラスター内のネームスペースを表します。バッキングネームスペースに保存されているリソースの例としては、{xref-secrets}#_creating_secrets_in_projects[プロジェクトスコープのシークレット（project scoped secrets）]や{xref-cluster-project-roles}#_project_roles[プロジェクトロールテンプレートバインディング（project role template bindings）]などがあります。
