...
在本示例中，我们将使用 https://prometheus.io/[Prometheus]。我们从以下假设开始：
...
为了让 HPA 使用 Prometheus 的自定义指标，您需要在集群的 `kube-system` 命名空间中安装软件包 https://github.com/DirectXMan12/k8s-prometheus-adapter[k8s-prometheus-adapter]。要安装`k8s-prometheus-adapter`，我们使用位于 https://github.com/banzaicloud/banzai-charts[banzai-charts]的Helm图表。
