...
. After the single nodes is up and running, Rancher recommends adding additional etcd nodes to your cluster. If you have a {xref-custom-clusters}[custom cluster] and you want to reuse an old node, you are required to xref:cluster-admin/manage-clusters/clean-cluster-nodes.adoc#_cleaning_up_nodes[clean up the nodes] before attempting to add them back into a cluster.
