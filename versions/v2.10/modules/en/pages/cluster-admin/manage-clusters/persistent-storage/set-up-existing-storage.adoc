...
The steps to set up a persistent storage device will differ based on your infrastructure. We provide examples of how to set up storage using {xref-vsphere-storage-example}[vSphere,] xref:cluster-admin/manage-clusters/persistent-storage/examples/nfs-storage.adoc[NFS,] or Amazon's xref:cluster-admin/manage-clusters/persistent-storage/examples/persistent-storage-in-amazon-ebs.adoc[EBS.]
