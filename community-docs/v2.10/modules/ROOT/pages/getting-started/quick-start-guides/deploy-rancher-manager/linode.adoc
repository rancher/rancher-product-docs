...
:community-path: getting-started/quick-start-guides/deploy-rancher-manager/linode
:product-path:
...
[CAUTION]
====
The intent of these guides is to quickly launch a sandbox that you can use to evaluate Rancher. These guides are not intended for production environments. For comprehensive setup instructions, see xref:getting-started/installation-and-upgrade/installation-and-upgrade.adoc[Installation].
====
...
* https://www.linode.com/[Linode Account]: The Linode account to run provision server and cluster under.
* https://techdocs.akamai.com/cloud-computing/docs/manage-personal-access-tokens[Linode Personal Access Token]: A Linode Personal Access Token to authenticate with.
* https://developer.hashicorp.com/terraform/install[Terraform]: Used to provision the server and cluster on Linode.
 ...
 * `rancher_server_admin_password` - Admin password for created Rancher server. See xref:getting-started/installation-and-upgrade/resources/bootstrap-password.adoc#_password_requirements[Setting up the Bootstrap Password] for password requirements.
 ...
 * `linode_type` - The type/plan that all infrastructure Linodes should use.
  ...
  ** For a complete list of plans, see the https://techdocs.akamai.com/cloud-computing/docs/compute-instance-plan-types[official Plan Types page].
. Run `terraform init`.
...
Use Rancher to create a deployment. For more information, see xref:getting-started/quick-start-guides/deploy-workloads/index.adoc[Creating Deployments].
