...
:community-path: getting-started/installation-and-upgrade/resources/choose-a-rancher-version
:product-path:

This section describes how to choose a Rancher version.

For a high-availability installation of Rancher, which is recommended for production, the Rancher server is installed using a **Helm chart** on a Kubernetes cluster. Refer to the xref:getting-started/installation-and-upgrade/resources/helm-version-requirements.adoc[Helm version requirements] to choose a version of Helm to install Rancher.
...
+
--
When installing, upgrading, or rolling back Rancher Server when it is xref:getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/index.adoc[installed on a Kubernetes cluster], Rancher server is installed using a Helm chart on a Kubernetes cluster. Therefore, as you prepare to install or upgrade a high availability Rancher configuration, you must add a Helm chart repository that contains the charts for installing Rancher.

Refer to the xref:getting-started/installation-and-upgrade/resources/helm-version-requirements.adoc[Helm version requirements] to choose a version of Helm to install Rancher.

[#_helm_chart_repositories]
[pass]
<h3><a class="anchor" id="_helm_chart_repositories" href="#_helm_chart_repositories"></a>Helm Chart Repositories</h3>
...
Instructions on when to select these repos are available below in <<_switching_to_a_different_helm_chart_repository,Switching to a Different Helm Chart Repository>>.
...
[#_helm_chart_versions]
[pass]
<h3><a class="anchor" id="_helm_chart_versions" href="#_helm_chart_versions"></a>Helm Chart Versions</h3>
...
[#_switching_to_a_different_helm_chart_repository]
[pass]
<h3><a class="anchor" id="_switching_to_a_different_helm_chart_repository" href="#_switching_to_a_different_helm_chart_repository"></a>Switching to a Different Helm Chart Repository</h3>
...
. Continue to follow the steps to xref:getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/upgrades.adoc[upgrade Rancher] from the new Helm chart repository.
...
+
--
When performing xref:getting-started/installation-and-upgrade/other-installation-methods/rancher-on-a-single-node-with-docker/index.adoc[Docker installs], upgrades, or rollbacks, you can use *tags* to install a specific version of Rancher.

[#_server_tags]
[pass]
<h3><a class="anchor" id="_server_tags" href="#_server_tags"></a>Server Tags</h3>
...
| You can install specific versions of Rancher by using the tag from a previous release. See what's available at Docker Hub.
