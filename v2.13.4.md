...
Rancher v2.13.4 is the latest patch release of Rancher. This is a Community version release that introduces maintenance updates and bug fixes.
...
- Fixed an issue where OIDC-based Auth Provider settings could be lost when upgrading from Rancher v2.12.x to v2.13.x or later. A cleanup process for unused OIDC secrets could cause the AuthConfig to be partially overwritten, leading to the loss of critical authentication settings like endpoints and client IDs. See [\#54053](https://github.com/rancher/rancher/issues/54053).

## Cluster Provisioning

### Major Bug Fixes

- Fixed an issue where provisioning or importing an Amazon EKS downstream cluster failed when the Rancher Server was running in an IPv6-only or dual-stack environment. See [\#52154](https://github.com/rancher/rancher/issues/52154).
...
- When installing the `rancher-turtles-providers` app from a Rancher Prime OCI registry, Rancher fails to render the chart correctly, resulting in a missing logo, an incorrect chart name, and an unexpected prompt to select a namespace and name. As a workaround, you can manually enter `cattle-turtles-system` for the namespace and `rancher-turtles-providers` for the name to proceed with the installation. See [\#53882](https://github.com/rancher/rancher/issues/53882).
...
See the full list of [changes](https://github.com/rancher/rancher/compare/v2.13.3%E2%80%A6v2.13.4).
...
- rancher/rancher:v2.13.4

### Tools

- CLI - [v2.13.4](https://github.com/rancher/cli/releases/tag/v2.13.4)
